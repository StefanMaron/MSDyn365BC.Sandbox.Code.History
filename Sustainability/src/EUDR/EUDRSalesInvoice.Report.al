...
/// <summary>
/// Generates a standard format printable document for posted sales invoices with EUDR lot certification details.
/// </summary>
report 6222 "EUDR Sales Invoice"
{
    Caption = 'EUDR Sales Invoice';
    EnableHyperlinks = true;
    Permissions = TableData "Sales Shipment Buffer" = rimd;
    PreviewMode = PrintLayout;
    WordMergeDataItem = Header;
    DefaultRenderingLayout = "EUDRSalesInvoice.rdlc";
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(DisplayAdditionalFeeNote; DisplayAdditionalFeeNoteVal)
            {
            }
            column(EUDRSalesInvoice_Lbl; EUDRSalesInvoiceLbl)
            {
            }
            column(PostingDate; Format("Posting Date"))
            {
            }
            column(PostingDate_Lbl; FieldCaption("Posting Date"))
            {
            }
            column(BillToName; "Bill-to Name")
            {
            }
            column(BillToName_Lbl; FieldCaption("Bill-to Name"))
            {
            }
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                dataitem(EUDRLot; "Lot No. Information")
                {
                    DataItemTableView = sorting("Item No.", "Variant Code", "Lot No.");
                    UseTemporary = true;
                    column(LotNo_EUDRLot; "Lot No.")
                    {
                    }
                    column(LotNo_EUDRLot_Lbl; FieldCaption("Lot No."))
                    {
                    }
                    column(LotQuantity_EUDRLot; GetEUDRLotQuantity(Line, EUDRLot))
                    {
                    }
                    column(LotQuantity_EUDRLot_Lbl; LotQuantityLbl)
                    {
                    }
                    column(CertificationScheme_EUDRLot; "Certification Scheme")
                    {
                    }
                    column(CertificationScheme_EUDRLot_Lbl; FieldCaption("Certification Scheme"))
                    {
                    }
                    column(EUDRCertificateNo_EUDRLot; "EUDR Certificate No.")
                    {
                    }
                    column(EUDRCertificateNo_EUDRLot_Lbl; FieldCaption("EUDR Certificate No."))
                    {
                    }

                    trigger OnPreDataItem()
                    begin
                        LoadEUDRLotsForLine(Line);
                    end;
                }
            ...
            dataitem(LineFee; "Integer")
            {
                ...
                trigger OnAfterGetRecord()
                begin
                    if not DisplayAdditionalFeeNoteVal then
                        CurrReport.Break();
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                ...
                {
                    Caption = 'Options';
                    field(LogInteraction; LogInteractionVal)
                    {
                    ...
                    field(DisplayShptInformation; DisplayShipmentInformation)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Show Shipments';
                        ToolTip = 'Specifies that shipments are shown on the document.';
                    }
                    field(DisplayAdditionalFeeNote; DisplayAdditionalFeeNoteVal)
                    {
                        ApplicationArea = Basic, Suite;
        ...
        trigger OnInit()
        begin
            LogInteractionVal := LogInteractionTemplateExists();
            LogInteractionEnable := LogInteractionVal;
        end;
    }

    rendering
    {
        layout("EUDRSalesInvoice.rdlc")
        {
            Type = RDLC;
            LayoutFile = 'src/EUDR/EUDRSalesInvoice.rdlc';
            Caption = 'EUDR Sales Invoice (RDLC)';
            Summary = 'Prints the standard sales invoice information with EUDR certification scheme and certificate number per line and lot.';
        }
    }
    ...
    begin
        if LogInteractionVal and not IsReportInPreviewMode() then
    ...
    var
        ...
        ProjectTaskNoLbl: Label 'Project Task No.';
        ...
        LCYTxt: label ' (LCY)';
        EUDRSalesInvoiceLbl: Label 'EUDR Sales Invoice';
        LotQuantityLbl: Label 'Quantity';
        VATClauseText: Text;
    ...
    protected var
        ...
        DisplayAdditionalFeeNoteVal: Boolean;
        FirstLineHasBeenOutput: Boolean;
        ShowShippingAddr: Boolean;
        TotalText: Text[50];
        LogInteractionVal: Boolean;
        TotalAmount: Decimal;
    ...
    local procedure LoadEUDRLotsForLine(SalesInvoiceLine: Record "Sales Invoice Line")
    var
    ...
    begin
        ...
        if not SalesInvoiceLine."EUDR Relevant" then
            exit;
            ...
            repeat
                if ItemLedgerEntry.Get(ValueEntry."Item Ledger Entry No.") then
                    if ItemLedgerEntry."Lot No." <> '' then
                        if LotNoInformation.Get(ItemLedgerEntry."Item No.", ItemLedgerEntry."Variant Code", ItemLedgerEntry."Lot No.") then
                            if not EUDRLot.Get(LotNoInformation."Item No.", LotNoInformation."Variant Code", LotNoInformation."Lot No.") then begin
                                EUDRLot := LotNoInformation;
                                EUDRLot.Insert();
                            end;
            until ValueEntry.Next() = 0;
    end;

    local procedure GetEUDRLotQuantity(SalesInvoiceLine: Record "Sales Invoice Line"; LotNoInformation: Record "Lot No. Information"): Decimal
    var
        ItemLedgerEntry: Record "Item Ledger Entry";
        ValueEntry: Record "Value Entry";
        LotQuantity: Decimal;
    begin
        ...
        if ValueEntry.FindSet() then
            repeat
                if ItemLedgerEntry.Get(ValueEntry."Item Ledger Entry No.") then
                    if (ItemLedgerEntry."Item No." = LotNoInformation."Item No.") and
                       (ItemLedgerEntry."Variant Code" = LotNoInformation."Variant Code") and
                       (ItemLedgerEntry."Lot No." = LotNoInformation."Lot No.")
                    then
                        LotQuantity -= ValueEntry."Invoiced Quantity";
            until ValueEntry.Next() = 0;
    ...
    begin
        LogInteractionVal := NewLogInteraction;
    ...
    local procedure GetJobTaskDescription(NewJobNo: Code[20]; NewJobTaskNo: Code[20]): Text[100]
    var
        JobTask: Record "Job Task";
    begin
        JobTask.SetRange("Job No.", NewJobNo);
        JobTask.SetRange("Job Task No.", NewJobTaskNo);

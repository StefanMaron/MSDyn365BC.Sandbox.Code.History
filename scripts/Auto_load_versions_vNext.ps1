...
Get-BCArtifactUrl -select All -Type Sandbox -country $country -accept_insiderEula -storageAccount bcinsider -after ([DateTime]::Today.AddDays(-2)) | % {
    [System.Uri]$Url = $_
...
$Versions | Sort-Object -Property Country, Version | % {
    ...
    git fetch origin "$($country)-$($Version.Major)-vNext" 2>&1 | Out-Null
    ...
    if ($LastCommit.Length -eq 0) {
        ...
        if (-not $TargetPathOfVersion -or -not (Test-Path $TargetPathOfVersion)) {
            Write-Host "##[warning]No Applications folder found for $($country)-$($version.ToString())-vNext, creating version marker commit only"
        ...
        if ($IsLateHotfix) {
            ...
            $RemoteBranchExists = git ls-remote --heads origin "$($country)-$($Version.Major)-vNext" 2>&1

...
codeunit 134686 "Email Accounts Test"
{
    ...
    begin
        ...
        Assert.IsTrue(AccountsPage.GoToKey(TempEmailAccount."Account Id", TempEmailAccount.Connector), 'The email account should be on the page');

        Assert.AreEqual(TempEmailAccount."Email Address", Format(AccountsPage.EmailAddress), 'The email address on the page is wrong');
        Assert.AreEqual(TempEmailAccount.Name, Format(AccountsPage.NameField), 'The account name on the page is wrong');
    ...
    begin
        ...
        Assert.IsTrue(AccountsPage.GoToKey(TempFirstEmailAccount."Account Id", Enum::"Email Connector"::"Test Email Connector"), 'The first email account should be on the page');
        Assert.AreEqual(TempFirstEmailAccount."Email Address", Format(AccountsPage.EmailAddress), 'The first email address on the page is wrong');
        Assert.AreEqual(TempFirstEmailAccount.Name, Format(AccountsPage.NameField), 'The first account name on the page is wrong');
        Assert.AreEqual(3, AccountsPage.EmailConcurrencyLimit.AsInteger(), 'The email concurrency limit on the page is wrong');
        Assert.AreEqual(0, AccountsPage.EmailRateLimit.AsInteger(), 'The email rate limit on the page is wrong');

        Assert.IsTrue(AccountsPage.GoToKey(TempSecondEmailAccount."Account Id", Enum::"Email Connector"::"Test Email Connector"), 'The second email account should be on the page');
        Assert.AreEqual(TempSecondEmailAccount."Email Address", Format(AccountsPage.EmailAddress), 'The second email address on the page is wrong');
        Assert.AreEqual(TempSecondEmailAccount.Name, Format(AccountsPage.NameField), 'The second account name on the page is wrong');
        Assert.AreEqual(3, AccountsPage.EmailConcurrencyLimit.AsInteger(), 'The email concurrency limit on the page is wrong');
        Assert.AreEqual(0, AccountsPage.EmailRateLimit.AsInteger(), 'The email rate limit on the page is wrong');

        // [When] The Email Rate Limit Wizard page is shown, the concurrency limit is changed to 5
        AccountsPage.GoToKey(TempFirstEmailAccount."Account Id", Enum::"Email Connector"::"Test Email Connector");
    ...
    var
        ...
        ConnectorMock: Codeunit "Connector Mock";
        TempEmailAccount: Codeunit "Email Account";
    begin
        ...
        ConnectorMock.Initialize();
        ConnectorMock.AddAccount(TempEmailAccountRecord);

        PermissionsMock.Set('Email Edit');

        // [Then] The email account is registered
        Assert.IsTrue(TempEmailAccount.IsAccountRegistered(TempEmailAccountRecord."Account Id", TempEmailAccountRecord.Connector), 'The email account should be registered');
    end;
    ...
    var
        TempEmailAccountBuffer, TempEmailAccounts : Record "Email Account";
        ConnectorMock: Codeunit "Connector Mock";
        TempEmailAccount: Codeunit "Email Account";
    begin
        ...
        TempEmailAccount.GetAllAccounts(TempEmailAccounts);

        // [THEN] The returned record is empty (there are no registered accounts)
        Assert.IsTrue(TempEmailAccounts.IsEmpty(), 'Record should be empty');

        // [GIVEN] An account is added to the connector
        ConnectorMock.AddAccount(TempEmailAccountBuffer);

        // [WHEN] GetAllAccounts is called
        TempEmailAccount.GetAllAccounts(TempEmailAccounts);

        // [THEN] The returned record is not empty and the values are as expected
        Assert.AreEqual(1, TempEmailAccounts.Count(), 'Record should not be empty');
        TempEmailAccounts.FindFirst();
        Assert.AreEqual(TempEmailAccountBuffer."Account Id", TempEmailAccounts."Account Id", 'Wrong account ID');
        Assert.AreEqual(Enum::"Email Connector"::"Test Email Connector", TempEmailAccounts.Connector, 'Wrong connector');
        Assert.AreEqual(TempEmailAccountBuffer.Name, TempEmailAccounts.Name, 'Wrong account name');
        Assert.AreEqual(TempEmailAccountBuffer."Email Address", TempEmailAccounts."Email Address", 'Wrong account email address');
    end;
    ...
    begin
        ...
        Assert.IsFalse(TempEmailAccount.IsAnyAccountRegistered(), 'There should be no registered accounts');
        ...
        Assert.IsTrue(TempEmailAccount.IsAnyAccountRegistered(), 'There should be a registered account');
    end;
    ...
    var
        TempSecondAccount: Record "Email Account";
        ConnectorMock: Codeunit "Connector Mock";
        EmailAccountsSelectionMock: Codeunit "Email Accounts Selection Mock";
    ...
    begin
        ...
        ConnectorMock.Initialize();
        ConnectorMock.AddAccount(FirstAccountId);
        ConnectorMock.AddAccount(TempSecondAccount);
        ConnectorMock.AddAccount(ThirdAccountId);

        // [GIVEN] The second account is set as default
        EmailScenario.SetDefaultEmailAccount(TempSecondAccount);
        ...
        Assert.IsTrue(EmailAccountsTestPage.GoToKey(TempSecondAccount."Account Id", Enum::"Email Connector"::"Test Email Connector"), 'The second email account should be on the page');
    ...
    begin
        ...
        BindSubscription(EmailAccountsSelectionMock);
        EmailAccountsSelectionMock.SelectAccount(TempSecondAccount."Account Id");
        EmailAccountsSelectionMock.SelectAccount(ThirdAccountId);
        ...
        Assert.IsFalse(EmailAccountsTestPage.GoToKey(TempSecondAccount."Account Id", Enum::"Email Connector"::"Test Email Connector"), 'The second email account should not be on the page');
        Assert.IsFalse(EmailAccountsTestPage.GoToKey(ThirdAccountId, Enum::"Email Connector"::"Test Email Connector"), 'The third email account should not be on the page');
    end;
    ...
    begin
        ...
        EmailAccountMgt.GetAllAccounts(TempEmailAccountToDelete);
        ...
        EmailAccountMgt.DeleteAccounts(TempEmailAccountToDelete, true);

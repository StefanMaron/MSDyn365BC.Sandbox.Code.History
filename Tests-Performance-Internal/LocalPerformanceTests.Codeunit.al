codeunit 139094 "Local Performance Tests"
{
    ...
    [Scope('OnPrem')]
    procedure TestSalesTaxesCollected()
    var
    ...
    begin
        ...
        PopulateTaxJurisdiction(TaxJurisdiction);

        LibraryPerformanceProfiler.StartProfiler(true);
        Report.Run(Report::"Sales Taxes Collected", true, false, TaxJurisdiction);
        TraceDumpFilePath := LibraryPerformanceProfiler.StopProfiler(
            PerfProfilerEventsTest, 'TestSalesTaxesCollected',
            PerfProfilerEventsTest."Object Type"::Report, REPORT::"Sales Taxes Collected", true);
    end;

    local procedure PopulateTaxJurisdiction(var TaxJurisdiction: Record "Tax Jurisdiction")
    var
    ...
    begin
        ...
        MockVATEntry(VATEntry[2], TaxDetail[2], LibraryRandom.RandDecInRange(100, 200, 2), true);
        ...
        TaxJurisdiction.SetFilter(Code, '%1|%2|%3',
    ...
    begin
        ...
        GLAccount.Modify();
        TaxJurisdiction."Tax Account (Sales)" := GLAccount."No.";
        TaxJurisdiction."Tax Account (Purchases)" := GLAccount."No.";
        TaxJurisdiction."Reverse Charge (Purchases)" := GLAccount."No.";
        TaxJurisdiction."Report-to Jurisdiction" := TaxJurisdiction.Code;
        TaxJurisdiction."Calculate Tax on Tax" := true;

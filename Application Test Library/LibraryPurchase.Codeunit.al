codeunit 130512 "Library - Purchase"
{
    ...
    begin
        ...
        if PurchaseHeader."Operation Type" = '' then
            PurchaseHeader.Validate("Operation Type", LibraryERM.GetDefaultOperationType(PurchaseHeader."Buy-from Vendor No.", DATABASE::Vendor));
        if (PurchaseHeader."Applies-to Doc. No." <> '') and (PurchaseHeader."Applies-to Occurrence No." = 0) then
            PurchaseHeader.Validate("Applies-to Occurrence No.", 1);
        if PurchaseHeader."Operation Occurred Date" = 0D then
            PurchaseHeader.Validate("Operation Occurred Date", PurchaseHeader."Posting Date");
        PurchaseHeader.Modify(true);
        SetCheckTotalOnPurchaseDocument(PurchaseHeader, false, NewShipReceive, NewInvoice);
            ...
            PurchaseHeader."Document Type"::Invoice, PurchaseHeader."Document Type"::"Credit Memo":
                if PurchaseHeader.Invoice and (PurchaseHeader."Posting No. Series" <> '') then begin
                    if (PurchaseHeader."Posting No." = '') then
                        PurchaseHeader."Posting No." := NoSeries.GetNextNo(PurchaseHeader."Posting No. Series", LibraryUtility.GetNoSeriesLastUsedDate(PurchaseHeader."Posting No. Series"));
                ...
                begin
                    if PurchaseHeader.Receive and (PurchaseHeader."Receiving No. Series" <> '') then begin
                        if (PurchaseHeader."Receiving No." = '') then
                            PurchaseHeader."Receiving No." := NoSeries.GetNextNo(PurchaseHeader."Receiving No. Series", LibraryUtility.GetNoSeriesLastUsedDate(PurchaseHeader."Receiving No. Series"));
                ...
                begin
                    if PurchaseHeader.Ship and (PurchaseHeader."Return Shipment No. Series" <> '') then begin
                        if (PurchaseHeader."Return Shipment No." = '') then
                            PurchaseHeader."Return Shipment No." := NoSeries.GetNextNo(PurchaseHeader."Return Shipment No. Series", LibraryUtility.GetNoSeriesLastUsedDate(PurchaseHeader."Return Shipment No. Series"));
    ...
    begin
        if PurchaseHeader."Document Type" in [PurchaseHeader."Document Type"::Order, PurchaseHeader."Document Type"::"Return Order"] then
            ...
            if PurchaseHeader.Invoice and not ShipReceive and not Prepayment then
                // Invoice only
            ...
            PurchaseHeader."Document Type"::Order, PurchaseHeader."Document Type"::"Return Order":
                begin
                    ...
                    if Prepayment then
                        ...
                        if ShipReceive and PurchaseHeader.Invoice then begin
                            if PurchaseHeader."Prices Including VAT" then
        ...
        PurchaseHeader.Validate("Check Total", Abs(TotalToInvoice));
        PurchaseHeader.Modify(true);
    end;

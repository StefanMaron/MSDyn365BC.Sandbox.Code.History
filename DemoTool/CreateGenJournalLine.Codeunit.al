codeunit 101081 "Create Gen. Journal Line"
{
    ...
    local procedure InsertBankVoucher(
    ...
    begin
        ...
        GenJnlLine."Line No." := GetNextLineNo(TemplateName, BatchName);
        GenJnlLine.Insert();
        ...
        GenJnlLine."TDS Section Code" := SectionCode;
        if LocationCode <> '' then
            GenJnlLine.Validate("Location Code", LocationCode);
        if (GenJnlLine."Account Type" = GenJnlLine."Account Type"::Customer) and (GenJnlLine."TDS Section Code" <> '') then begin
            GenJnlLine."TDS Certificate Receivable" := true;
            if Loc.Get(LocationCode) then
                GenJnlLine."T.A.N. No." := Loc."T.A.N. No.";
        end;
        if NOC <> '' then
            GenJnlLine."T.C.A.N. No." := 'DELN03830B';
        GenJnlLine.Validate("Bal. Account Type", BalAccType);
        GenJnlLine.Validate("Bal. Account No.", BalAccNo);
        if CurrencyCode <> '' then
            GenJnlLine.validate("Currency Code", CurrencyCode);
        GenJnlLine.Validate("External Document No.", ExternalDocumentNo);
        GenJnlLine."TCS Nature of Collection" := NOC;
        GenJnlLine."Nature of Remittance" := NOR;
        GenJnlLine."Act Applicable" := Act;
        if GSTGrp <> '' then
            GenJnlLine.Validate("GST Group Code", GSTGrp);
        if HSNCode <> '' then
            GenJnlLine.Validate("HSN/SAC Code", HSNCode);
        if GSTOnAdv then
            GenJnlLine.Validate("GST on Advance Payment", GSTOnAdv);
        GenJnlLine.Modify();
    ...
    local procedure InsertCashReceiptVoucher(
      TemplateName: Text[20];
      BatchName: Text[20];
      DocumentType: Enum "Gen. Journal Document Type";
      DocumentNo: Code[20];
    ...
    begin
        ...
        if LocationCode <> '' then
            GenJnlLine.Validate("Location Code", LocationCode);
        if NOC <> '' then
            GenJnlLine."T.C.A.N. No." := 'RED0897580';
        GenJnlLine.Validate("Bal. Account Type", BalAccType);
        GenJnlLine.Validate("Bal. Account No.", BalAccNo);
        if CurrencyCode <> '' then
            GenJnlLine.validate("Currency Code", CurrencyCode);
    ...
    local procedure GetNextLineNo(TemplateName: Code[20]; BatchName: Text[20]): Integer
    var
        GenJnlLine: Record "Gen. Journal Line";
    begin
    ...
    procedure UpdateGenJurLine(DocNO: Code[20]; JnlTempName: Code[10]; GSTGrpCode: Code[20]; HSNSAC: Code[20];
      GSTOnAdvPaym: Boolean; LocationCode: Code[20]; ExternalDoc: Code[20])
    var
        GenJournalLine: Record "Gen. Journal Line";
    begin
        if GenJournalLine.Get(DocNO) then begin
            GenJournalLine.Validate("GST Group Code", GSTGrpCode);
            GenJournalLine.Validate("Journal Template Name", JnlTempName);
            GenJournalLine."Line No." := 10000;
            GenJournalLine.Validate("HSN/SAC Code", HSNSAC);
            GenJournalLine.Validate("GST on Advance Payment", GSTOnAdvPaym);
            GenJournalLine.Validate("Location Code", LocationCode);
            GenJournalLine.Validate("External Document No.", ExternalDoc);
            GenJournalLine.Modify();
        end;
    end;

    procedure CreateINGenJnlLines()
    var
        PostingDate: Date;
    begin
        PostingDate := CA.AdjustDate(19030102D);
        SourceCodeSetup.Get();

        InsertBankVoucher('BANKPYMTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BP-00001', PostingDate, "Gen. Journal Account Type"::Vendor, '10000', '', '', '', '', SourceCodeSetup."Bank Payment Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-V-00001', 10000, '', 'BLUE', true, '2090', '2090001', false);
        InsertBankVoucher('BANKPYMTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BP-00002', PostingDate, "Gen. Journal Account Type"::Vendor, '10000', '', '', '', '', SourceCodeSetup."Bank Payment Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-V-00002', 2000, '', 'BLUE', true, '2090', '2090001', true);
        InsertBankVoucher('BANKPYMTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BP-00003', PostingDate, "Gen. Journal Account Type"::Vendor, '30000', '194J-PF', '', '', '', SourceCodeSetup."Bank Payment Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-V-00003', 10000, '', 'BLUE', false, '', '', true);
        InsertBankVoucher('BANKPYMTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BP-00004', PostingDate, "Gen. Journal Account Type"::Vendor, '30000', '194I-LB', '', '', '', SourceCodeSetup."Bank Payment Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-V-00004', 2000, '', 'BLUE', false, '', '', false);
        InsertBankVoucher('BANKPYMTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BP-00005', PostingDate, "Gen. Journal Account Type"::Vendor, '40000', '195', '', '16', 'A', SourceCodeSetup."Bank Payment Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-V-00005', 100, 'USD', 'RED', false, '', '', false);

        InsertBankVoucher('BANKRCPTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BR-00001', PostingDate, "Gen. Journal Account Type"::Customer, '10000', '', '', '', '', SourceCodeSetup."Bank Receipt Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-C-00001', -10000, '', 'BLUE', true, '2089', '2089001', false);
        InsertBankVoucher('BANKRCPTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BR-00002', PostingDate, "Gen. Journal Account Type"::Customer, '10000', '', '', '', '', SourceCodeSetup."Bank Receipt Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-C-00002', -2000, '', 'BLUE', true, '2089', '2089001', true);
        InsertBankVoucher('BANKRCPTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BR-00003', PostingDate, "Gen. Journal Account Type"::Customer, '40000', 'S', '', '', '', SourceCodeSetup."Bank Receipt Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-C-00003', -10000, '', 'BLUE', false, '', '', false);
        InsertBankVoucher('BANKRCPTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BR-00004', PostingDate, "Gen. Journal Account Type"::Customer, '50000', '', '', '', '', SourceCodeSetup."Bank Receipt Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-C-00004', -12000, '', 'BLUE', false, '', '', true);
        InsertBankVoucher('BANKRCPTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'BR-00005', PostingDate, "Gen. Journal Account Type"::Customer, '50000', '', '', '', '', SourceCodeSetup."Bank Receipt Voucher", "Gen. Journal Account Type"::"Bank Account", 'GIRO', 'EX-DOC-C-00005', -9755, 'USD', 'BLUE', false, '', '', false);

        InsertCashReceiptVoucher('CASHRCPTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'CR00001', PostingDate, "Gen. Journal Account Type"::Customer, '50000', 'A', SourceCodeSetup."Cash Receipt Voucher", "Gen. Journal Account Type"::"G/L Account", '2910', 'EX-DOC-C-00011', -12000, 'USD', '', false);
        InsertCashReceiptVoucher('CASHRCPTV', 'USER-A', "Gen. Journal Document Type"::Payment, 'CR00002', PostingDate, "Gen. Journal Account Type"::Customer, '50000', 'A', SourceCodeSetup."Cash Receipt Voucher", "Gen. Journal Account Type"::"G/L Account", '2910', 'EX-DOC-C-00012', -2000, '', 'RED', true);
    end;
}

codeunit 101081 "Create Gen. Journal Line"
{
    ...
    begin
        ...
        InsertDailyEntry(
          XGENERAL, 3, XWWBUSD, 19030119D, 0, XxBANK2, XBankTransfer,
        ...
        InsertDailyEntry(
          XGENERAL, 0, '998210', 19030125D, 0, '2602', XOfficeSupplies,
        ...
        InsertBankAccEntry(XNBL, XxBANK2, -1);
    ...
    var
        XSET4Q93: Label 'SET-4Q93';
        XxBANK2: Label 'BANK2';
        XOfficeSupplies: Label 'Office Supplies';
        XDiffOnDailyEntries1on2PERC: Label 'Difference on Daily entries: %1 on %2\\';
        XDoYouWantToContinue: Label 'Do you want to continue?';
        XProgramTerminatedByTheUser: Label 'Program terminated by the user';
        XDiffOnOpeningEntries1on2PERC: Label 'Difference on Opening entries: %1 on %2\\';
        XDiffOnPeriodicEntries1on2PERC: Label 'Difference on Periodic entries: %1 on %2\\';
        XDiffOnOtherEntries1on2PERC: Label 'Difference on Other entries: %1 on %2\\';
        ...
        DocsCountInStartingYear: Integer;
    ...
    begin
        ...
        // IT - the first days of the first week can be in previous year (i.e. for 2019 this is a 31-12-2018)
        // There is a new IT Series No. starting from new ("Starting Year" + 1) year
        // Count number of docs posted in previous year and shift new IT series by this number
        if Date2DMY(Date, 3) = DemoDataSetup."Starting Year" then
            DocsCountInStartingYear += 1;
            ...
            if "Exactly Balanced" then begin
                if Abs(("Entry Balance" / Amount + 1) * 100) > 1 then
                    if not
                       Confirm(
                         StrSubstNo(
                           XDiffOnDailyEntries1on2PERC +
                           XDoYouWantToContinue, "Entry Balance" + Amount, Date),
                         true)
                    then
                        Error(XProgramTerminatedByTheUser);
            ...
            '2806':
                begin
                    ...
                    GenJournalLine.Validate("Applies-to Doc. No.",
                      CopyStr(Format(Date2DMY(CA.AdjustDate(DMY2Date(1, 1, 1903)), 3), 4), 3, 2) +
                      '-C0100' + Format(11 - DocsCountInStartingYear, 0, '<Integer,2><Filler Character,0>')); // IT - "01-C010011" for DocsCountInStartingYear = 0
                end;
            '2807':
                begin
                    ...
                    GenJournalLine.Validate("Applies-to Doc. No.",
                      CopyStr(Format(Date2DMY(CA.AdjustDate(DMY2Date(1, 1, 1903)), 3), 4), 3, 2) +
                      '-C0100' + Format(12 - DocsCountInStartingYear, 0, '<Integer,2><Filler Character,0>')); // IT - "01-C010012" for DocsCountInStartingYear = 0
                end;
        end;
        case "Document No." of
            '2701':
        ...
        GenJournalLine."Payment Method Code" := ''; // IT
    ...
    begin
        ...
        GenJournalLine."Payment Method Code" := ''; // IT
        GenJournalLine."Allow Issue" := false; // IT
        GenJournalLine.Insert();
    end;
        ...
        if "Exactly Balanced" then begin
            if Abs(("Entry Balance" / Amount + 1) * 100) > 1 then
                if not
                   Confirm(
                     StrSubstNo(
                       XDiffOnOpeningEntries1on2PERC +
                       XDoYouWantToContinue, "Entry Balance" + Amount, GenJournalLine."Posting Date"),
                     true)
                then
                    Error(XProgramTerminatedByTheUser);
            Amount := -"Entry Balance";
        ...
        if "Exactly Balanced" then begin
            if Abs(("Entry Balance" / Amount + 1) * 100) > 1 then
                if not
                   Confirm(
                     StrSubstNo(
                       XDiffOnPeriodicEntries1on2PERC +
                       XDoYouWantToContinue, "Entry Balance" + Amount, Date),
                     true)
                then
                    Error(XProgramTerminatedByTheUser);

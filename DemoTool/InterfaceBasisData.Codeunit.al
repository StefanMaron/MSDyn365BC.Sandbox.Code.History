codeunit 110000 "Interface Basis Data"
{
    ...
    var
        ...
        JobNo: Code[20];
        XCreatingBillGroups: Label 'Creating Bill Groups';
        XCreatingPaymentOrders: Label 'Creating Payment Orders';
        FiscalYearClose: Codeunit "Fiscal Year-Close";
    ...
    begin
        ...
        RunCodeunit(CODEUNIT::"Create Port/Airport");
        ...
        RunCodeunit(CODEUNIT::"Create Cartera Setup");
        ...
        RunCodeunit(CODEUNIT::"Create AEAT Trans. Form. lines");
        ...
        RunCodeunit(CODEUNIT::"Create Doc.-Handling Fee");
    ...
    begin
        ...
        if PostingDate = CA.AdjustDate(19021231D) then begin
            ...
            AccPeriod.SetFilter("Starting Date", '< %1', PostingDate);
            AccPeriod.SetRange("New Fiscal Year", true);
            if AccPeriod.Find('-') then
                repeat
                    FiscalYearClose.SetHidePrintDialog(true);
                    FiscalYearClose.Run(AccPeriod);
                    GLEntry.SetRange("Posting Date", AccPeriod."Starting Date", CalcDate('<+1Y-1D>', AccPeriod."Starting Date"));
                    REPORT.Run(REPORT::"Set Period Trans. Nos.", false, true, GLEntry);
                until (AccPeriod."Starting Date" >= PostingDate);
            Commit();

            CODEUNIT.Run(CODEUNIT::"Run Close Income Statement");
            GLEntry.SetRange("Posting Date", PostingDate, CalcDate('<+1Y-1D>', PostingDate));
            REPORT.Run(REPORT::"Set Period Trans. Nos.", false, true, GLEntry);
    ...
    begin
        ...
        CODEUNIT.Run(CODEUNIT::"Create Bill Group");
        CODEUNIT.Run(CODEUNIT::"Create Payment Order");

codeunit 119085 "Create Cost Types"
{
    ...
    begin
        ...
        CostType.SetRange("No.", MakeAdjustments.Convert('997280'), MakeAdjustments.Convert('997193'));
        CostType.DeleteAll();
        CostType.SetRange("No.", MakeAdjustments.Convert('997291'), MakeAdjustments.Convert('997293'));
        CostType.DeleteAll();
        CostType.Reset();

        CostType."No." := MakeAdjustments.Convert('997280');
        CostType.Name := xPurchaseTrade;
        CostType."G/L Account Range" := MakeAdjustments.Convert('997280') + '..' + MakeAdjustments.Convert('997193');
        CostType."Cost Center Code" := XMATERIAL;
        if CostType.Modify() then;

        CostType."No." := MakeAdjustments.Convert('997291');
        CostType.Name := XPurchaseRawMat;
        CostType."G/L Account Range" := MakeAdjustments.Convert('997291') + '..' + MakeAdjustments.Convert('997293');
        CostType."Cost Center Code" := XMATERIAL;
        if CostType.Modify() then;

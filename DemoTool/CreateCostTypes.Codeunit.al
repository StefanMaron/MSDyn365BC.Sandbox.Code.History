codeunit 119085 "Create Cost Types"
{
    ...
    var
        ...
        XPERS: Label 'PERS', Comment = 'PERS for Person and it is a name of cost center.';
        ...
        XACTACCR: Label 'ACTACCR', Comment = 'ACTACCR stands for Actual Accurals and it is name of cost center.';
        XEXPENSES: Label 'EXPENSES', Comment = 'Expenses is a name of Cost Type.';
        XCOSTS: Label 'COSTS';
        XEXP: Label 'Expense';
        XCOS: Label 'Costs';
        XEXPE: Label 'expense';
    ...
    begin
        ...
        if CostObjectCode <> '' then begin
            if not CostObject.Get(CostObjectCode) then
                Error(XCostCostCenterNoOpen, CostObjectCode);
    ...
    begin
        ...
        // CostType.DeleteAll();
        CostType.SetRange("No.", MakeAdjustments.Convert('983020'), MakeAdjustments.Convert('997130'));
        CostType.DeleteAll();
        CostType.SetRange("No.", MakeAdjustments.Convert('983210'), MakeAdjustments.Convert('997230'));
        CostType.DeleteAll();
        CostType.Reset();

        // CostType."No." := MakeAdjustments.Convert('997110');
        CostType."No." := MakeAdjustments.Convert('983010');
        CostType.Name := xPurchaseTrade;
        // CostType."G/L Account Range" := MakeAdjustments.Convert('997110') + '..' + MakeAdjustments.Convert('997193');
        CostType."G/L Account Range" := MakeAdjustments.Convert('983020') + '..' + MakeAdjustments.Convert('997130');
        CostType."Cost Center Code" := XMATERIAL;
        if CostType.Modify() then;

        // CostType."No." := MakeAdjustments.Convert('997210');
        CostType."No." := MakeAdjustments.Convert('983210');
        CostType.Name := XPurchaseRawMat;
        // CostType."G/L Account Range" := MakeAdjustments.Convert('997210') + '..' + MakeAdjustments.Convert('997293');
        CostType."G/L Account Range" := MakeAdjustments.Convert('983220') + '..' + MakeAdjustments.Convert('997230');
        CostType."Cost Center Code" := XMATERIAL;
        if CostType.Modify() then;

codeunit 101098 "Create General Ledger Setup"
{
    ...
    begin
        ...
        with "General Ledger Setup" do begin
            ...
            "Local Address Format" := "Local Address Format"::"City+County+Post Code (no comma)";
            "Show Amounts" := "Show Amounts"::"Amount Only";
            "Post Dated Journal Template" := XPOSTDATED;
            "Post Dated Journal Batch" := XDEFAULT;
            "Address Validation" := "Address Validation"::"Post Code & City";
            "Enable GST (Australia)" := true;
            "Adjustment Mandatory" := true;
            "BAS GST Division Factor" := 11;
            "Enable WHT" := true;
            "Min. WHT Calc only on Inv. Amt" := true;
            "GST Report" := true;
            "Full GST on Prepayment" := true;
            Modify();
        end;

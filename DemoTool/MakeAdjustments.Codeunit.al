codeunit 101902 "Make Adjustments"
{
    ...
    begin
        ...
        if DemoDataSetup."Data Type" <> DemoDataSetup."Data Type"::Extended then
            exit(ConvertForMiniApp(Return));
    end;

    procedure ConvertForMiniApp(AccountNo: Code[20]): Code[20]
    var
        TmpNumber: Integer;
    begin
        // For Mini, the chart of accounts is reversed.
        // Income statement comes first from 1000 to 5999
        // Balance sheet is second from 6000 to 9999
        // Below code just offsets the numbers from regular demo tool to these new values
        // 6000 becomes 1000 (and up)
        // 1000 becomes 5000 (and up)
        if not Evaluate(TmpNumber, AccountNo) then
            exit(AccountNo);

        if TmpNumber > 9999 then
            exit(AccountNo);

        if TmpNumber <= 5999 then
    ...
    begin
        ...
        AddMapElement('995613', '5613');
        AddMapElement('995633', '5633');
    end;

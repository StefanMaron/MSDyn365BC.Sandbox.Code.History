codeunit 118016 "Create Transfer Order Add."
{
    ...
    procedure CreateINTransferOrder()
    var
        TransferHeader: Record "Transfer Header";
        TransferNo: Code[20];
    begin
        TransferNo := InsertHeader(XRED, XBLUE, XOWNLOG,
            CA.AdjustDate(19030126D), CA.AdjustDate(19030126D),
            0D);
        TransferHeader.get(TransferNo);
        TransferHeader."External Document No." := 'OPEN';
        TransferHeader.modify();
        InsertLine(TransferNo, 10000, '1936-S', 1, 1, 0);
        InsertLine(TransferNo, 20000, '1968-S', 1, 1, 0);
        UpdateTransferPriceGST(TransferNo, 10000, 100);
        UpdateTransferPriceGST(TransferNo, 20000, 100);
    end;

codeunit 101997 "Create Item Template"
{
    ...
    var
        ...
        xITEMTxt: Label 'ITEM', Comment = 'Stands for Item, keep capitalized.';
        xITEMNOVATTxt: Label 'ITEMNOVAT', Comment = 'Stands for Item No VAT, keep capitalized.';
        xSERVICETxt: Label 'SERVICE', Comment = 'Stands for Service, keep capitalized.';
        xSERVNOVATTxt: Label 'SERVNOVAT', Comment = 'Stands for Service No VAT, keep capitalized.';
    ...
    begin
        with DemoDataSetup do begin
            ...
            // Item template
            InsertTemplateHeader(ConfigTemplateHeader, xItemDescrTxt, xITEMTxt);
            ...
            InsertTemplateHeader(ConfigTemplateHeader, xItemNoVATDescrTxt, xITEMNOVATTxt);
            InsertTemplateLines(ConfigTemplateHeader, Format(Item.Type::Inventory), xBaseUOMPCSTxt, NoVATCode, NoVATCode, ResaleCode);
            InsertPricingInfo(ConfigTemplateHeader, false);

            // Service template
            InsertTemplateHeader(ConfigTemplateHeader, xServiceDescTxt, xSERVICETxt);
            ...
            InsertTemplateHeader(ConfigTemplateHeader, xServiceNoVATDescTxt, xSERVNOVATTxt);
            InsertTemplateLines(ConfigTemplateHeader, Format(Item.Type::Service), xBaseUOMHourTxt, ServicesCode, NoVATCode, '');
            InsertPricingInfo(ConfigTemplateHeader, false);
    ...
    local procedure InsertTemplateHeader(var ConfigTemplateHeader: Record "Config. Template Header"; Description: Text[50]; TemplateHeaderCode: Code[10])
    begin
        CreateTemplateHelper.CreateTemplateHeader(
          ConfigTemplateHeader, TemplateHeaderCode, Description, DATABASE::Item);
    end;

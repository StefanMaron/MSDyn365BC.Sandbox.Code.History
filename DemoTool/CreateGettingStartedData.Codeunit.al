codeunit 101975 "Create Getting Started Data"
{
    ...
    procedure ImportDemoDataFiles()
    var
        DemoDataSetup: Record "Demo Data Setup";
        TempNameValueBuffer: Record "Name/Value Buffer" temporary;
        DemoDataFiles: Record "Demo Data File";
        FileManagement: Codeunit "File Management";
    begin
        DemoDataSetup.Get();
        FileManagement.GetServerDirectoryFilesList(TempNameValueBuffer, TemporaryPath() + '..\DemoDataFiles\');
        TempNameValueBuffer.SetFilter(Name, '*.json');

        if not TempNameValueBuffer.FindSet() then
            exit;

        DemoDataFiles.DeleteAll();
        repeat
            DemoDataFiles.Init();
            DemoDataFiles.Type := DemoDataFiles.Type::Evaluation;
            DemoDataFiles.Version := CopyStr(FileManagement.GetFileNameWithoutExtension(TempNameValueBuffer.Name), 1);
            DemoDataFiles.Insert();
            DemoDataFiles.SetJsonFile(TempNameValueBuffer.Name);
        until TempNameValueBuffer.Next() = 0
    end;
}

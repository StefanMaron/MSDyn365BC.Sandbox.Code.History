codeunit 101156 "Create Resource"
{
    ...
    begin
        ...
        ModifyData(XLIFT, XGSTGroup2089, XHSNCode2089001);
    end;
    ...
    local procedure ModifyData(ResourceNo: Code[20]; GSTGroupCode: Code[20]; HSNSacCode: Code[20])
    var
        Resource: Record Resource;
    begin
        if Resource.Get(ResourceNo) then begin
            Resource."GST Group Code" := GSTGroupCode;
            Resource."HSN/SAC Code" := HSNSacCode;
            Resource.Modify();
        end;

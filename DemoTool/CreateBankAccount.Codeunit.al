codeunit 101270 "Create Bank Account"
{
    ...
    begin
        ...
        if DemoDataSetup."Data Type" = DemoDataSetup."Data Type"::Extended then begin
            InsertData(
              ...
              XHollyDickson, '', XLCY, '', '', '05004312560000567450', '');
            InsertData(
            ...
            InsertData(
              ...
              XGrantCulbertson, '', XOPERATING, '', '', '00401212450006778766', '');
            InsertData(
            ...
            InsertData(
              ...
              XPaulaNartker, '', XLCY2, '', '', '00121201220004556678', XGB80RBOS16173241116737);
        end else begin
            InsertData(
    ...
    procedure InsertData("No.": Code[20]; Name: Text[30]; Address: Text[30]; "Post Code": Text[30]; Contact: Text[30]; "Currency Code": Code[10]; "Posting Group": Code[20]; "Bank Account No.": Text[30]; "Bank Branch No.": Text[20]; "CCC No.": Text[20]; IBAN: Code[50])
    var
    ...
    begin
        ...
        BankAcc.County := CreatePostCode.FindCounty("Post Code");
        ...
        BankAcc.Validate("CCC No.", "CCC No.");
        ...
        case BankAcc."No." of
            ...
            XCHECKING:
                begin
                    ...
                    BankAcc."Credit Limit for Discount" := 5000000;
                    BankAcc."Delay for Notices" := 5;
                    if DemoDataSetup."Data Type" = DemoDataSetup."Data Type"::Extended then begin
                        BankAcc."Operation Fees Code" := XWWBOPERATING;
                        BankAcc."Customer Ratings Code" := XWWBOPERATING;
                    end;
                end;
            XNBL:
                ...
                begin
                    BankAcc."Last Statement No." := '4';
                    BankAcc."Credit Limit for Discount" := 25000000;
                    BankAcc."Delay for Notices" := 2;
                    BankAcc."Operation Fees Code" := XNBL;
                    BankAcc."Customer Ratings Code" := XNBL;
                end;
        end;

codeunit 101270 "Create Bank Account"
{
    ...
    begin
        ...
        if DemoDataSetup."Data Type" = DemoDataSetup."Data Type"::Extended then begin
            InsertData(
              ...
              XHollyDickson, '', XLCY2, '78-66-345', 'NB54366', ''); // NAVCZ
            InsertData(
            ...
            InsertData(
              ...
              XGrantCulbertson, '', XLCY, '99-99-888', 'BG99999', ''); // NAVCZ
            InsertData(
            ...
            InsertData(
              ...
              XPaulaNartker, '', XLCY, '14-55-678', 'GO284033', XGB80RBOS16173241116737); // NAVCZ
        end else begin
            // NAVCZ
            InsertData(
              XNBL, XNewBankofLondon, X4BakerStreet, CreatePostCode.Convert('GB-W1 3AL'),
              XHollyDickson, '', XNBL, '158316358/0100', '', '');
            SetAsBalAccOnGenJnlBatch(XNBL);
            InsertData(
              XWWBEUR, XWorldWideBank, X1HighHolborn, CreatePostCode.Convert('GB-WC1 3DG'),
              XGrantCulbertson, XEUR, XWWB, '99-33-456', 'BG99999', 'CZ4401000000351623420217');
            // NAVCZ
        end;
    end;
    ...
    var
        ...
        XDEFAULT: Label 'DEFAULT', Comment = 'Code of bank payment application rules';
        XWWB: Label 'WWB', Comment = 'World Wide Bank';
    ...
    begin
        ...
        if DemoDataSetup."Data Type" = DemoDataSetup."Data Type"::Evaluation then begin
            BankAcc."Dimension from Apply Entry CZB" := true;
            BankAcc."Variable S. to Variable S. CZB" := true;
            BankAcc."Variable S. to Ext.Doc.No. CZB" := true;
            BankAcc."Post Per Line CZB" := true;
            BankAcc."Payment Partial Suggestion CZB" := true;
            BankAcc."Non Assoc. Payment Account CZB" := '261900';
            BankAcc."Domestic Payment Order ID CZB" := Report::"Iss. Payment Order CZB";

            case BankAcc."No." of
                XWWBEUR:
                    BankAcc."Foreign Payment Orders CZB" := true;
                XNBL:
                    BankAcc.Validate(
                      "Min. Balance",
                      Round(
        ...
        if BankAcc."No." = XNBL then
            BankAcc."Use as Default for Currency" := true;
        // NAVCZ

        BankAcc.Validate("Our Contact Code", XOF);

        BankAcc.Modify(true); // NAVCZ
    end;
    ...
    procedure GetBankAccountCode(BankAccountCode: Text): Code[20]
    begin
        case UpperCase(BankAccountCode) of
            'XNBL':
                exit(XNBL);
            'XWWBEUR':
                exit(XWWBEUR);

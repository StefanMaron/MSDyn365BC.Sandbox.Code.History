codeunit 101270 "Create Bank Account"
{
    ...
    begin
        ...
        if DemoDataSetup."Data Type" = DemoDataSetup."Data Type"::Extended then begin
            ...
            ModifyData(XNBL, DemoDataSetup."Country/Region Code", '110001', 'DL', '', BankRegStatus::" ");
            InsertData(
            ...
            ModifyData(XGIRO, DemoDataSetup."Country/Region Code", '110001', 'DL', X07LOCAT1000R1Z1, BankRegStatus::Registered);
        end else begin
            InsertData(
            ...
            InsertData(
              XNBL, XNewBankofLondon, X4BakerStreet, CreatePostCode.Convert('GB-W1 3AL'),
              XHollyDickson, '', XCHECKING, '78-66-345', 'NB54366', '');
            ModifyData(XNBL, DemoDataSetup."Country/Region Code", '110001', 'DL', '', BankRegStatus::" ");
            InsertData(
              XGIRO, XGiroBank, X2BridgeStreet, CreatePostCode.Convert('GB-W1 3AL'),
              XPaulaNartker, '', XSAVINGS, '14-55-678', 'GO284033', XGB80RBOS16173241116737);
            ModifyData(XGIRO, DemoDataSetup."Country/Region Code", '110001', 'DL', X07LOCAT1000R1Z1, BankRegStatus::Registered);
        end;
    end;
    ...
    var
        ...
        X07LOCAT1000R1Z1: Label '07LOCAT1000R1Z1';
        BankRegStatus: Enum "Bank Registration Status";
    ...
    local procedure ModifyData(BankAccNo: Code[20]; CountryCode: Code[10]; PostCode: Code[20]; StateCode: Code[10]; GSTRegistrationNo: Code[20]; GSTRegistrationStatus: Enum "Bank Registration Status")
    var
        BankAcc: Record "Bank Account";
    begin
        if BankAcc.get(BankAccNo) then begin
            BankAcc."State Code" := StateCode;
            BankAcc."Country/Region Code" := CountryCode;
            BankAcc.Validate("Post Code", PostCode);
            BankAcc."GST Registration No." := GSTRegistrationNo;
            BankAcc."GST Registration Status" := GSTRegistrationStatus;
            BankAcc.Modify();
        end;
    end;
}

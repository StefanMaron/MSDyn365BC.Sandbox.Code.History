codeunit 101018 "Create Customer"
{
    ...
    begin
        ...
        if DemoDataSetup."Data Type" = DemoDataSetup."Data Type"::Evaluation then begin
            Customer."Print Statements" := true;
            Customer."Payment Method Code" := CreatePaymentMethod.GetBankCode();

            if Customer."No." = '50000' then begin
                Customer."Payment Terms Code" := CreatePaymentTerms.CashOnDeliveryCode();
                Customer."Payment Method Code" := CreatePaymentMethod.GetCashCode();
            end;
        end;
        // NAVCZ
    ...
    begin
        ...
        InsertData(
          ...
          DemoDataSetup."Country/Region Code", '', 'CZ789456278', '', '', '');
        UpdateContact('10000', XRobertTownes);
        InsertData(
          '20000', XTreyResearch, "X91-95SouthwarkBridgeRd", CreatePostCode.Convert('GB-SE1 0AX'), '', '', '',
          DemoDataSetup."Country/Region Code", '', 'CZ733495789', '', '', '');
        UpdateContact('20000', XHelenRay);
        InsertData(
          '30000', XSchoolOfFineArt, X10HighTowerGreen, CreatePostCode.Convert('US-FL 37125'), '', '', 'USD',
          'US', '', '', '', '', '');
        UpdateContact('30000', XMeaganBond);
        InsertData(
          '40000', XAlpineSkiHouse, X5WalterGropiusStrasse, CreatePostCode.Convert('DE-80807'), '', '', 'EUR',
          'DE', '', '456124966B25', '', XParkStadtSchwabing, '');

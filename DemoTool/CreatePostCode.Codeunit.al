codeunit 101225 "Create Post Code"
{
    ...
    begin
        ...
        InsertData('IS-810', XReykjavik, '');
        InsertData('IS-600', XReykjavik, '');
        InsertData('IS-540', XCity1, '');
        InsertData('IS-999', XCity2, '');
        InsertData('IS-126', XReykjavik, '');
        InsertData('IS-130', XReykjavik, '');
        InsertData('IS-640', XReykjavik, '');
        InsertData('IS-310', XReykjavik, '');
        InsertData('IS-110', XReykjavik, '');
        InsertData('IS-120', XReykjavik, '');
        InsertData('IS-131', XReykjavik, '');
        InsertData('IS-780', XCity3, '');
        InsertData('IS-900', XCity4, '');
        InsertData('IS-245', XCity5, '');
        InsertData('IS-210', XCity6, '');
    ...
    var
        ...
        XCity1: Label 'Blönduós';
        XCity2: Label 'Vesturvík';
        XCity3: Label 'Höfn í Hornafirði';
        XCity4: Label 'Vestmannaeyjar';
        XCity5: Label 'Sandgerði';
        XCity6: Label 'Garðabæ';
    ...
    begin
        case PostCode of
            ...
            'GB-BS3 6KL':
                Return := 'IS-200'; // Location xYellow
            'GB-CV6 1GY':
                Return := 'IS-640'; // Customer 20000
            'GB-DY5 4DJ':
                Return := 'IS-810'; // Ship-toAdress 10000/xDudley
            'GB-EH16 8JS':
                Return := 'GB-EH16 8JS'; // AltAddress xMH
            'GB-GL1 9HM':
                Return := 'IS-310'; // Customer 40000
            'GB-GU3 2SE':
                Return := 'IS-108'; // Vendor 20000 | OrderAddress 20000/xJAMES
            'GB-GU7 5GT':
                Return := 'IS-220'; // Customer 50000
            'GB-IB7 7VN':
                Return := 'IS-120'; // Vendor 40000
            'GB-L18 6SA':
                Return := 'IS-200'; // Location xGreen
            'GB-M61 2YG':
                Return := 'IS-600'; // Contact xCT100239|xCT100125|xCT100187|xCT100188
            'GB-MO2 4RT':
                Return := 'IS-600'; // Customer 30000 | Ship-toAdress 20000/xMANCHESTER
            ...
            'GB-N12 5XY':
                Return := 'IS-101';
            'GB-N16 34Z':
                Return := 'IS-101'; // Resource xMary | Employee xMD/xLM
            'GB-NP5 6GH':
                Return := 'IS-780'; // Contact xCT100240|xCT100124|xCT100147|xCT100176
            'GB-OX16 0UA':
                Return := 'IS-900'; // Contact xCT100241|xCT100213|CT200035|xCT200130
            'GB-PE17 4RN':
                Return := 'IS-101'; // Employee xAH
            'GB-PO21 6HG':
                Return := 'IS-245'; // Contact xCT100242|xCT100134|xCT100142|CT200037
            'GB-PO7 2HI':
                Return := 'IS-110'; // Vendor 30000
            'GB-TN27 6YD':
                Return := 'IS-600'; // Location xRed | Ship-toAdress 20000/xEASTACTON
            'GB-TQ17 8HB':
                Return := 'IS-210'; // Contact xCT100243|xCT100126|xCT100127
            'GB-W1 3AL':
                Return := 'IS-126'; // BankAccount xNBL|XGIRO | Employee xPS/xJR/xRL / Contact xCT100237
            'GB-W2 8HG':
                Return := 'IS-999'; // Company Information
            'GB-WC1 2GS':
                Return := 'IS-540'; // Location xWhite|xSilver
            'GB-WC1 3DG':
                Return := 'IS-130'; // BankAccount xWWBEUR|XWWBOPERATING|XWWBUSD | Contact xCT100159|xCT100170
            'GB-WD2 4RG':
                Return := 'IS-131'; // Vendor 50000 | OrderAddress 20000/xWATFORD | Contact xCT100162|xCT200046|xCT200068
            'GB-RG6 1WG':
                Return := 'IS-131'; // Main wharehouse
            'GB-EC2A 3JL':
                Return := 'IS-131'; // East wharehouse
            'GB-NP10 8BE':
                Return := 'IS-131'; // West wharehouse
            'GB-GL50 1TY':
                Return := 'IS-131'; // Customer 10000 Eval
            'GB-W2 6BD':
                Return := 'IS-131'; // Customer 10000 Eval
            'GB-GU52 8DY':
                Return := 'IS-131'; // Customer 20000
            'GB-CV9 3QN':
                Return := 'IS-131'; // Customer 20000
            'US-FL 37125':

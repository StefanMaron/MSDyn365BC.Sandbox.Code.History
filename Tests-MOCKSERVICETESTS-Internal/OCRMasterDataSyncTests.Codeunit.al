codeunit 135098 "OCR Master Data Sync Tests"
{
    ...
    [Scope('OnPrem')]
    procedure TestSyncRequestContainsVendorABN()
    var
        Vendor: Record Vendor;
        ReadSoftOCRMasterDataSync: Codeunit "ReadSoft OCR Master Data Sync";
        LastSyncTime: DateTime;
    begin
        // [FEATURE] [Vendor] [ABN]
        // [SCENARIO 372895] OCR "Sync Master Data" request contains vendor's ABN number in "TaxRegistrationNumber" node
        Initialize(true, true, false);
        LastSyncTime := CurrentDateTime();
        SetLastSyncTime(LastSyncTime);
        Sleep(100);
        ...
        DeleteJobQueueEntry();
        OCRMasterDataSyncEvents.EnableValidation();
        OCRMasterDataSyncEvents.EnqueueVariable(StrSubstNo('<TaxRegistrationNumber>%1</TaxRegistrationNumber>', Vendor.ABN));
        OCRMasterDataSyncEvents.EnqueueVariable('');

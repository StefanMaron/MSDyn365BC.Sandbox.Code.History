...
codeunit 90 "Purch.-Post"
{
    ...
    begin
        ...
        SetTaxType(PurchHeader, TempPurchLineGlobal);

        if TaxOption = TaxOption::SalesTax then begin
            SalesTaxCalculationOverridden := false;
            OnBeforeCalculateSalesTax(
              PurchHeader, TempPurchLineForSalesTax, TempSalesTaxAmtLine, SalesTaxCalculationOverridden, SuppressCommit);
            if not SalesTaxCalculationOverridden then
                if PurchaseHeader2."Tax Area Code" <> '' then begin
                    if UseExternalTaxEngine then
                        SalesTaxCalculate.CallExternalTaxEngineForPurch(PurchHeader, false)
                    else
                        SalesTaxCalculate.EndSalesTaxCalculation(PurchaseHeader2."Posting Date");
...
#pragma warning restore AA0470
        ...
        TempProvSalesTaxAmtLine: Record "Sales Tax Amount Line" temporary;
...
#pragma warning restore AA0470
        ...
        if PurchLine.Quantity <> 0 then begin
            ...
            if not IsHandled then
                if GLSetup."VAT in Use" then begin
                    ...
                    if not (PurchLine.Type in [PurchLine.Type::"Fixed Asset", PurchLine.Type::"G/L Account"]) then
                        PurchLine.TestField("Gen. Prod. Posting Group");
            IsHandled := false;
    ...
    var
        ...
        DummyLineCount: Integer;
#endif
        TotalAmount: Decimal;
        IsHandled: Boolean;
        TotalUseTaxAmount: Decimal;
        TotalNotUseTaxAmount: Decimal;
    ...
    begin
        ...
        if TaxOption = TaxOption::SalesTax then
            if PurchHeader."Tax Area Code" <> '' then begin
                PostSalesTaxToGL(PurchHeader, TotalUseTaxAmount, TotalNotUseTaxAmount);
                if PurchHeader.Invoice then
                    TaxAmountDifference.ClearDocDifference(
                        Enum::"Sales Tax Document Area"::Purchase.AsInteger(), PurchHeader."Document Type".AsInteger(), PurchHeader."No.");
                PostProvincialSalesTaxToGL(PurchHeader, TotalUseTaxAmount);

                NewAmountIncludingVAT :=
                  TotalPurchLineLCY.Amount + TotalPurchLineLCY."Tax To Be Expensed" + TotalNotUseTaxAmount - TotalUseTaxAmount;
                if PurchHeader.Invoice and PurchHeader."Prepmt. Include Tax" and (PurchHeader."Prepayment %" <> 0) and (PurchHeader."Currency Code" = '') and
                   ((TotalUseTaxAmount <> 0) or (TotalNotUseTaxAmount <> 0)) and (NewAmountIncludingVAT <> 0) and
                   (TotalPurchLineLCY."Amount Including VAT" <> 0) and (TotalPurchLineLCY."Amount Including VAT" <> NewAmountIncludingVAT)
                then
                    PostSalesTaxToGLRounding(
                      PurchHeader,
                      NewAmountIncludingVAT - TotalPurchLineLCY."Amount Including VAT",
                      NewAmountIncludingVAT - TotalPurchLineLCY."Amount Including VAT");
                // Tax rounding
                if (TotalUseTaxAmount <> 0) or (TotalNotUseTaxAmount <> 0) then begin
                    if ((TotalPurchLineLCY."Amount Including VAT" - TotalPurchLineLCY.Amount) <> 0) and
                       ((TotalPurchLineLCY."Amount Including VAT" - TotalPurchLineLCY.Amount) <> TotalNotUseTaxAmount)
                    then
                        TotalPurchLineLCY."Amount Including VAT" := NewAmountIncludingVAT;
...
#if not CLEAN23
            OnPostGLAndVendorOnBeforePostBalancingEntry(PurchHeader, DummyPurchLine, TempInvoicePostBuffer, DummyLineCount);
            if UseLegacyInvoicePosting() then
                PostBalancingEntry(
                    PurchHeader, TotalPurchLine, TotalPurchLineLCY, GenJnlLineDocType, GenJnlLineDocNo, GenJnlLineExtDocNo, SrcCode)
            else
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if TaxOption = TaxOption::SalesTax then
                ItemJnlLine2.Amount +=
                  ItemChargePurchLine."Tax To Be Expensed" * TempItemChargeAssgntPurch."Qty. to Assign" / ItemChargeQtyToAssign(ItemChargePurchLine);
...
#pragma warning restore AL0432
    ...
    begin
        ...
        if PurchHeader.Invoice and (TaxOption = TaxOption::SalesTax) and UseExternalTaxEngine then
    ...
    begin
        ...
        if (PurchLineQty = 0) or (PurchLine."Direct Unit Cost" = 0) and not
            SalesTaxCalculate.HasExciseTax(PurchLine."Tax Area Code", PurchLine."Tax Group Code", PurchLine."Tax Liable", PurchLine.Quantity, PurchHeader."Document Date") then begin
            ...
            PurchLine."Tax To Be Expensed" := 0;
            ...
            if PurchLine."VAT Calculation Type" = PurchLine."VAT Calculation Type"::"Sales Tax" then begin
                if (QtyType = QtyType::Invoicing) and
                   TempPurchLineForSalesTax.Get(PurchLine."Document Type", PurchLine."Document No.", PurchLine."Line No.")
                then begin
                    PurchLine."Line Amount" := TempPurchLineForSalesTax."Line Amount";
                    PurchLine."Line Discount Amount" := TempPurchLineForSalesTax."Line Discount Amount";
                    PurchLine.Amount := TempPurchLineForSalesTax.Amount;
                    PurchLine."Amount Including VAT" := TempPurchLineForSalesTax."Amount Including VAT";
                    PurchLine."Inv. Discount Amount" := TempPurchLineForSalesTax."Inv. Discount Amount";
                    PurchLine."VAT Base Amount" := TempPurchLineForSalesTax."VAT Base Amount";
                    PurchLine."Tax To Be Expensed" := TempPurchLineForSalesTax."Tax To Be Expensed";
                end else begin
                    PurchLine."Line Amount" := Round(PurchLineQty * PurchLine."Direct Unit Cost", Currency."Amount Rounding Precision");
                    if PurchLineQty <> PurchLine.Quantity then
                        PurchLine."Line Discount Amount" :=
                          Round(PurchLine."Line Amount" * PurchLine."Line Discount %" / 100, Currency."Amount Rounding Precision");
                    ...
                    if PurchLine."Allow Invoice Disc." then
                        if QtyType = QtyType::Invoicing then
                            ...
                            TempPurchLineForSpread."Inv. Discount Amount" :=
                              TempPurchLineForSpread."Inv. Discount Amount" +
                              PurchLine."Inv. Discount Amount" * PurchLineQty / PurchLine.Quantity;
                            PurchLine."Inv. Discount Amount" :=
                              Round(TempPurchLineForSpread."Inv. Discount Amount", Currency."Amount Rounding Precision");
                            TempPurchLineForSpread."Inv. Discount Amount" :=
                              TempPurchLineForSpread."Inv. Discount Amount" - PurchLine."Inv. Discount Amount";
                        end;
                ...
                if not IsHandled then
                    TempVATAmountLine.Get(
                      PurchLine."VAT Identifier", PurchLine."VAT Calculation Type", PurchLine."Tax Group Code", PurchLine."Tax Area Code", PurchLine."Use Tax",
                      PurchLine."Line Amount" >= 0);
...
#pragma warning restore AA0005
    ...
    begin
        ...
        SalesTaxExists :=
            (PurchaseLine.Amount <> PurchaseLine."Amount Including VAT") or (PurchaseLine."Tax To Be Expensed" <> 0) or SalesTaxExists;
    ...
    begin
        ...
        PurchaseLine."Tax To Be Expensed" :=
            Round(
                CurrExchRate.ExchangeAmtFCYToLCY(
                    PurchaseHeader.GetUseDate(), PurchaseHeader."Currency Code",
                    TotalPurchLine."Tax To Be Expensed", PurchaseHeader."Currency Factor")) -
    ...
    begin
        ...
        PurchLine."Tax To Be Expensed" := -PurchLine."Tax To Be Expensed";
    ...
    begin
        ...
        if InvoiceRoundingAmount <> 0 then begin
            ...
            PurchLine."Tax Area Code" := '';
            PurchLine."Tax Liable" := false;
    ...
    begin
        ...
        Increment(TotalPurchLine."Tax To Be Expensed", PurchLine."Tax To Be Expensed");
    ...
    begin
        ...
        OnSumPurchLinesTempOnAfterCalcVATAmount(PurchHeader, TotalPurchLine, OldPurchLine, VATAmount);
    ...
    var
        ...
        AssignError: Boolean;
    begin
        OnBeforeCopyAndCheckItemCharge(PurchHeader);
        ...
        OnCopyAndCheckItemChargeOnBeforeThrowError(PurchHeader, InvoiceEverything, AssignError);
    ...
    begin
        ...
        if PurchLineToPost."Tax To Be Expensed" <> 0 then
            PurchLineToPost."Tax To Be Expensed" :=
              Round(PurchLineToPost."Tax To Be Expensed" * TempItemChargeAssgntPurch."Qty. to Assign" / PurchLineToPost."Qty. to Invoice");
                    ...
                    if TempLineFound then begin
                        ...
                        if PurchHeader."Prepmt. Include Tax" then
                            TempPrepmtPurchLine.Validate(
                              "Direct Unit Cost",
                              TempPrepmtPurchLine."Direct Unit Cost" + TempPurchLine."Prepmt Amt to Deduct" * (1 + TempPurchLine."VAT %" / 100))
                        else
                            TempPrepmtPurchLine.Validate(
                        ...
                        if PurchHeader."Prepmt. Include Tax" then
                            TempPrepmtPurchLine."Prepmt Amt to Deduct" += TempPurchLine.CalcAmountIncludingTax(TempPurchLine."Prepmt Amt to Deduct")
                        else
                        ...
                        if PurchHeader."Prepmt. Include Tax" then
                            TempPrepmtPurchLine.Validate("Direct Unit Cost", TempPurchLine."Prepmt Amt to Deduct" * (1 + TempPurchLine."VAT %" / 100))
                        else
    ...
    begin
        ...
        if PurchHeader."Prepmt. Include Tax" then
            TempPrepmtDeductLCYPurchLine."Prepmt Amt to Deduct" := TempPrepmtDeductLCYPurchLine.CalcAmountIncludingTax(PurchLine."Prepmt Amt to Deduct");
    ...
    begin
        ...
        if PrepmtPurchLine.FindSet() then
            repeat
                ...
                UpdatePrepmtAmountInvBuf(PrepmtPurchLine."Line No.", ActualCurrencyFactor, PurchHeader."Prepmt. Include Tax");
        ...
        if TempPrepmtDeductLCYPurchLine.FindSet(true) then
            repeat
                if PrepmtIncludeTax then
                    TempPrepmtDeductLCYPurchLine."Prepmt. Amount Inv. (LCY)" := TempPrepmtDeductLCYPurchLine."Prepmt Amt to Deduct"
                else
        ...
        if PrepmtPurchLine."Prepayment Line" then begin
            ...
            if TempPrepmtDeductLCYPurchLine.FindSet(true) then begin
                ...
                repeat
                    ...
                    if (not PurchHeader."Prepmt. Include Tax") and (PurchLine."Qty. to Invoice" <> TempPrepmtDeductLCYPurchLine."Qty. to Invoice") then
                    ...
                    if TempPrepmtDeductLCYPurchLine."VAT Calculation Type" <> TempPrepmtDeductLCYPurchLine."VAT Calculation Type"::"Full VAT" then
                        if PurchHeader."Prepmt. Include Tax" and not TempPrepmtDeductLCYPurchLine.IsFinalInvoice() then
                            TotalPrepmtAmount[1] += TempPrepmtDeductLCYPurchLine."Prepmt Amt to Deduct"
                        else
    ...
    begin
        ...
        if TempPrepmtDeductLCYPurchLine.Get(PurchaseLine."Document Type", PurchaseLine."Document No.", PurchaseLine."Line No.") then begin
            if PurchaseHeader."Prepmt. Include Tax" then
    ...
    begin
        ...
        if TempPurchLineForSalesTax."Qty. per Unit of Measure" = 0 then
            TempPurchLineForSalesTax."Qty. per Unit of Measure" := 1;
        if TempPurchLineForSalesTax."Document Type" in [TempPurchLineForSalesTax."Document Type"::"Return Order", TempPurchLineForSalesTax."Document Type"::"Credit Memo"] then begin
            if TempPurchLineForSalesTax."Return Shipment No." <> '' then begin
                TempPurchLineForSalesTax."Return Qty. Shipped" := TempPurchLineForSalesTax.Quantity;
                TempPurchLineForSalesTax."Return Qty. Shipped (Base)" := TempPurchLineForSalesTax."Quantity (Base)";
                TempPurchLineForSalesTax."Return Qty. to Ship" := 0;
                TempPurchLineForSalesTax."Return Qty. to Ship (Base)" := 0;
            end;
            MaxInvQty := (TempPurchLineForSalesTax."Return Qty. Shipped" - TempPurchLineForSalesTax."Quantity Invoiced");
            MaxInvQtyBase := (TempPurchLineForSalesTax."Return Qty. Shipped (Base)" - TempPurchLineForSalesTax."Qty. Invoiced (Base)");
            if PurchHeader.Ship then begin
                MaxInvQty := MaxInvQty + TempPurchLineForSalesTax."Return Qty. to Ship";
                MaxInvQtyBase := MaxInvQtyBase + TempPurchLineForSalesTax."Return Qty. to Ship (Base)";
            end;
        end else begin
            if TempPurchLineForSalesTax."Receipt No." = '' then begin
                MaxInvQty := (TempPurchLineForSalesTax."Quantity Received" - TempPurchLineForSalesTax."Quantity Invoiced");
                MaxInvQtyBase := (TempPurchLineForSalesTax."Qty. Received (Base)" - TempPurchLineForSalesTax."Qty. Invoiced (Base)");
            end else begin
                TempPurchLineForSalesTax."Quantity Received" := TempPurchLineForSalesTax.Quantity;
                TempPurchLineForSalesTax."Qty. Received (Base)" := TempPurchLineForSalesTax."Quantity (Base)";
                TempPurchLineForSalesTax."Qty. to Receive" := 0;
                TempPurchLineForSalesTax."Qty. to Receive (Base)" := 0;
                MaxInvQty := (TempPurchLineForSalesTax.Quantity - TempPurchLineForSalesTax."Quantity Invoiced");
                MaxInvQtyBase := (TempPurchLineForSalesTax."Quantity (Base)" - TempPurchLineForSalesTax."Qty. Invoiced (Base)");
            end;
            if PurchHeader.Receive then begin
                MaxInvQty := MaxInvQty + TempPurchLineForSalesTax."Qty. to Receive";
                MaxInvQtyBase := MaxInvQtyBase + TempPurchLineForSalesTax."Qty. to Receive (Base)";
            end;
        end;
        if Abs(TempPurchLineForSalesTax."Qty. to Invoice") > Abs(MaxInvQty) then begin
            TempPurchLineForSalesTax."Qty. to Invoice" := MaxInvQty;
            TempPurchLineForSalesTax."Qty. to Invoice (Base)" := MaxInvQtyBase;
        end;
        if TempPurchLineForSalesTax.Quantity = 0 then
            TempPurchLineForSalesTax."Inv. Disc. Amount to Invoice" := 0
        else
            TempPurchLineForSalesTax."Inv. Disc. Amount to Invoice" :=
              Round(
                TempPurchLineForSalesTax."Inv. Discount Amount" * TempPurchLineForSalesTax."Qty. to Invoice" / TempPurchLineForSalesTax.Quantity,
                Currency."Amount Rounding Precision");
        TempPurchLineForSalesTax."Quantity (Base)" := TempPurchLineForSalesTax."Qty. to Invoice (Base)";
        TempPurchLineForSalesTax."Line Amount" := Round(TempPurchLineForSalesTax."Qty. to Invoice" * TempPurchLineForSalesTax."Direct Unit Cost", Currency."Amount Rounding Precision");
        if TempPurchLineForSalesTax.Quantity <> TempPurchLineForSalesTax."Qty. to Invoice" then
            TempPurchLineForSalesTax."Line Discount Amount" :=
              Round(TempPurchLineForSalesTax."Line Amount" * TempPurchLineForSalesTax."Line Discount %" / 100, Currency."Amount Rounding Precision");
        TempPurchLineForSalesTax.Quantity := TempPurchLineForSalesTax."Qty. to Invoice";
        if (TempPurchLineForSalesTax."Prepayment %" = 100) and not TempPurchLineForSalesTax."Prepayment Line" and (TempPurchLineForSalesTax."Prepmt Amt to Deduct" <> 0) then
            TempPurchLineForSalesTax."Line Amount" := TempPurchLineForSalesTax."Prepmt Amt to Deduct" + TempPurchLineForSalesTax."Inv. Disc. Amount to Invoice"
        else
            TempPurchLineForSalesTax."Line Amount" := TempPurchLineForSalesTax."Line Amount" - TempPurchLineForSalesTax."Line Discount Amount";
        TempPurchLineForSalesTax."Inv. Discount Amount" := TempPurchLineForSalesTax."Inv. Disc. Amount to Invoice";
        TempPurchLineForSalesTax.Amount := TempPurchLineForSalesTax.CalcLineAmount();
        TempPurchLineForSalesTax."VAT Base Amount" := TempPurchLineForSalesTax.Amount;
        OnAddSalesTaxLineToSalesTaxCalcOnBeforeOnBeforeTempPurchLineForSalesTaxInsert(TempPurchLineForSalesTax, PurchHeader);
        TempPurchLineForSalesTax.Insert();
    ...
    local procedure PostSalesTaxToGL(PurchHeader: Record "Purchase Header"; var TotalUseTaxAmount: Decimal; var TotalNotUseTaxAmount: Decimal)
    var
        ...
        if TempSalesTaxAmtLine.Find('-') then begin
            repeat
                ...
                then begin
                    ...
                    GenJnlLine."VAT Reporting Date" := PurchHeader."VAT Reporting Date";
                    ...
                    GenJnlLine."GST/HST" := PurchHeader."GST/HST Tax Type";

                    OnPostSalesTaxToGLOnAfterGenJnlLineAssignField(GenJnlLine, PurchHeader, TempPurchLineGlobal);
                    ...
                    if GenJnlLine."Use Tax" then
                        ...
                        TotalNotUseTaxAmount += GenJnlLine."VAT Amount (LCY)";

                    OnPostSalesTaxToGLOnBeforeGenJnlPostLine(GenJnlLine, PurchHeader, TempSalesTaxAmtLine);
                    GenJnlPostLine.RunWithCheck(GenJnlLine);
                end;
            ...
            then begin
                PostSalesTaxToGLRounding(PurchHeader, -TotalPurchLine."Amount Including VAT", -TotalPurchLineLCY."Amount Including VAT");

                TotalPurchLine."Amount Including VAT" := 0;
                TotalPurchLineLCY."Amount Including VAT" := 0;
                TotalNotUseTaxAmount := 0;
            end;
        end;
    end;

    local procedure PostSalesTaxToGLRounding(PurchaseHeader: Record "Purchase Header"; LineAmount: Decimal; LineAmountLCY: Decimal)
    var
    ...
    begin
        ...
        GenJournalLine."Account No." := VendorPostingGroup."Invoice Rounding Account";
        ...
        GenJournalLine."Source Currency Amount" := -TotalPurchLine."Amount Including VAT";
    ...
    var
        ...
        TaxJurisdiction: Record "Tax Jurisdiction";
    begin
        OnBeforeCalcProvincialSalesTax(PurchHeader);
    ...
    begin
        ...
        if TempProvSalesTaxAmtLine.FindSet() then
            repeat
                ...
                then begin
                    ...
                    OnPostProvincialSalesTaxToGLOnAfterGenJnlLineAssignFields(GenJnlLine, PurchHeader);
                    ...
                    if TempProvSalesTaxAmtLine."Tax Amount" <> 0 then begin
                        RemSalesTaxSrcAmt := RemSalesTaxSrcAmt +
                          TempProvSalesTaxAmtLine."Tax Base Amount FCY" * TempProvSalesTaxAmtLine."Tax %" / 100;
                        GenJnlLine."Source Curr. VAT Amount" := Round(RemSalesTaxSrcAmt, Currency."Amount Rounding Precision");
                        RemSalesTaxSrcAmt := RemSalesTaxSrcAmt - GenJnlLine."Source Curr. VAT Amount";
        ...
        OnAfterPostProvincialSalesTaxToGL(PurchHeader, TotalUseTaxAmount, GenJnlLineDocNo)
    end;
    ...
    begin
        if GLSetup."VAT in Use" then
            ...
            if (PurchLine.Type.AsInteger() >= PurchLine.Type::"G/L Account".AsInteger()) and
    ...
    begin
        ...
        if TempPurchLine.FindSet() then
            repeat
                ...
                OnPostUpdateInvoiceLineOnBeforePurchOrderLineModify(PurchaseHeader, PurchOrderLine);
    ...
    begin
        TaxOption := 0;
        if PurchHeader.Invoice then begin
            PurchLine.SetRange("Prepayment Line", false);
            ...
            PurchLine.SetRange("Prepayment Line");
        end;
        if IsInterfaceInitialized then begin
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalculateSalesTax(PurchaseHeader: Record "Purchase Header"; var TempPurchLineForSalesTax: Record "Purchase Line" temporary; var TempSalesTaxAmountLine: Record "Sales Tax Amount Line" temporary; var IsHandled: Boolean; CommitIsSuppressed: Boolean)
    begin
    end;
...
#pragma warning restore AS0072
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostProvincialSalesTaxToGLOnAfterGenJnlLineAssignFields(var GenJnlLine: Record "Gen. Journal Line"; PurchaseHeader: Record "Purchase Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostSalesTaxToGLOnBeforeGenJnlPostLine(var GenJnlLine: Record "Gen. Journal Line"; PurchaseHeader: Record "Purchase Header"; var TempSalesTaxAmtLine: Record "Sales Tax Amount Line");
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostSalesTaxToGLOnAfterGenJnlLineAssignField(var GenJnlLine: Record "Gen. Journal Line"; PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAddSalesTaxLineToSalesTaxCalcOnBeforeOnBeforeTempPurchLineForSalesTaxInsert(var TempPurchLineForSalesTax: Record "Purchase Line" temporary; PurchHeader: Record "Purchase Header")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnSumPurchLinesTempOnAfterCalcVATAmount(PurchHeader: Record "Purchase Header"; TotalPurchLine: Record "Purchase Line"; OldPurchLine: Record "Purchase Line"; var VATAmount: Decimal)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostGLAndVendorOnBeforePostBalancingEntry(var PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; var TempInvoicePostBuffer: Record "Invoice Post. Buffer" temporary; var LineCount: Integer)
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostUpdateInvoiceLineOnBeforePurchOrderLineModify(var PurchHeader: Record "Purchase Header"; var PurchOrderLine: Record "Purchase Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCopyAndCheckItemCharge(var PurchaseHeader: Record "Purchase Header")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnCopyAndCheckItemChargeOnBeforeThrowError(varPurchHeader: Record "Purchase Header"; var InvoiceEverything: Boolean; var AssignError: Boolean)
    begin
    end;
}

...
codeunit 90 "Purch.-Post"
{
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            if IsCreditDocType() and GLSetup.GSTEnabled("Document Date") and GLSetup."Adjustment Mandatory" then
    ...
    begin
        with PurchaseHeader do begin
            Receive := "Document Type" in ["Document Type"::Order, "Document Type"::Invoice];
            Ship := "Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"];
            Invoice := true;
        end;
    end;
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            "Vendor Exchange Rate (ACY)" := PurchHeader."Vendor Exchange Rate (ACY)";
            ...
            if ShouldUpdateReceivingNo then
                ...
                then
                    if not PreviewMode then begin
                        ...
                        OnSetPostingPreviewDocumentNo("Receiving No.");
                    end;
            ...
            if Ship and ("Return Shipment No." = '') then
                ...
                then
                    if not PreviewMode then begin
                        ...
                        OnSetPostingPreviewDocumentNo("Return Shipment No.");
                    end;
            ...
            if not IsHandled then
                if Invoice and ("Posting No." = '') then begin
                    ...
                    if PreviewMode then begin
                        ...
                        OnSetPostingPreviewDocumentNo("Posting No.");
                    end;
            ...
            if (PurchLineQty = 0) or ("Direct Unit Cost" = 0) then begin
                ...
                if PurchHeader."Prices Including VAT" then begin
                    ...
                    if "VAT Calculation Type" = "VAT Calculation Type"::"Full VAT" then begin
                        ...
                        if FullGST and "Prepayment Line" then
                            "VAT Base Amount" := Round(TempVATAmountLine."VAT Base", Currency."Amount Rounding Precision")
                        else
                            "VAT Base Amount" :=
                              Round(
                        ...
                        if TempVATAmountLine."VAT Base" = 0 then begin
                            ...
                            if "Prepayment Line" and FullGST then begin
                                TempVATAmountLineRemainder."VAT Amount" +=
                                  TempVATAmountLine."VAT Amount" * CalcLineAmount() / TempVATAmountLine.CalcLineAmount();
                                TempVATAmountLineRemainder."VAT Amount (ACY)" +=
                                  TempVATAmountLine."VAT Amount (ACY)" * CalcLineAmount() / TempVATAmountLine.CalcLineAmount();
                            end else begin
                                TempVATAmountLineRemainder."VAT Amount" +=
                                  TempVATAmountLine."VAT Amount" * CalcLineAmount() / TempVATAmountLine.CalcLineAmount();
                                TempVATAmountLineRemainder."VAT Amount (ACY)" +=
                              ...
                              Round(
                                CurrExchRate.ExchangeAmtLCYToFCY(
                                  ...
                                  Round(CurrExchRate.ExchangeAmtFCYToLCY(
                                      PurchHeader."Posting Date", "Currency Code", Amount,
                                      PurchHeader."Currency Factor"), Currency."Amount Rounding Precision"), CurrencyFactor),
                                AddCurrency."Amount Rounding Precision");
                            "VAT Base (ACY)" :=
                              Round(
                                "Amount (ACY)" * (1 - PurchHeader."VAT Base Discount %" / 100), Currency."Amount Rounding Precision");
                            "Amount Including VAT (ACY)" :=
                              "Amount (ACY)" + Round(TempVATAmountLineRemainder."VAT Amount (ACY)", Currency."Amount Rounding Precision");
                            TempVATAmountLineRemainder."VAT Amount (ACY)" :=
                              TempVATAmountLineRemainder."VAT Amount (ACY)" - "Amount Including VAT (ACY)" + "Amount (ACY)";
                        end;
                    end;
...
#pragma warning restore AA0005
                ...
                if TotalPurchLine."Amount Including VAT (ACY)" <> 0 then begin
                    if (PurchHeader."Currency Code" <> GLSetup."Additional Reporting Currency") and (PurchHeader."Currency Code" <> '') then
                        InvoiceRoundingAmountACY := Round(CurrExchRate.ExchangeAmtLCYToFCY(
                              PurchHeader."Posting Date", GLSetup."Additional Reporting Currency",
                              Round(CurrExchRate.ExchangeAmtFCYToLCY(
                                  PurchHeader."Posting Date", "Currency Code", InvoiceRoundingAmount,
                                  PurchHeader."Currency Factor"), Currency."Amount Rounding Precision"), CurrencyFactor),
                            AddCurrency."Amount Rounding Precision")
                    else
                        if (PurchHeader."Currency Code" = GLSetup."Additional Reporting Currency") and (PurchHeader."Currency Code" <> '') then
                    ...
                    Validate("Amount Including VAT (ACY)", InvoiceRoundingAmountACY);
                end;
                "Line No." := BiggestLineNo;
    ...
    begin
        with PurchLine do begin
            ...
            Increment(TotalPurchLine."VAT Base (ACY)", "VAT Base (ACY)");
            Increment(TotalPurchLine."VAT Difference (ACY)", "VAT Difference (ACY)");
            Increment(TotalPurchLine."Amount Including VAT (ACY)", "Amount Including VAT (ACY)");
            Increment(TotalPurchLine."Amount (ACY)", "Amount (ACY)");
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            "WHT Business Posting Group" := TempPurchLineGlobal."WHT Business Posting Group";
            "WHT Product Posting Group" := TempPurchLineGlobal."WHT Product Posting Group";
            "Vendor Exchange Rate (ACY)" := PurchHeader."Vendor Exchange Rate (ACY)";
            "System-Created Entry" := true;
    ...
    begin
        ...
        with GenJnlLine do begin
            if TotalWHTAmount <> 0 then begin
                Amount := -(TotalPurchLine."Amount Including VAT" - TotalWHTAmount);
            ...
            "Amount Including VAT (ACY)" := -TotalPurchLineLCY2."Amount Including VAT (ACY)";
        ...
        with GenJnlLine do begin
            ...
            if GenJnlTemplate.FindFirst() then begin
                ...
                if GenJnlBatch.FindFirst() then
                    Validate("Journal Batch Name", GenJnlBatch.Name);
            end;
            "WHT Business Posting Group" := TempPurchLineGlobal."WHT Business Posting Group";
            "WHT Product Posting Group" := TempPurchLineGlobal."WHT Product Posting Group";
                        ...
                        if TempLineFound then begin
                            ...
                            if GLSetup.CheckFullGSTonPrepayment("VAT Bus. Posting Group", "VAT Prod. Posting Group") then
                                TempPrepmtPurchLine."VAT Base Amount" := TempPrepmtPurchLine."VAT Base Amount" +
                                  Round(
                                    TempPrepmtPurchLine.Quantity * ("Prepmt. VAT Base Amt." * "Qty. to Invoice" / Quantity),
                                    Currency."Amount Rounding Precision");
                            ...
                            FillPrepmtLineNoBuf("Line No.", TempPrepmtPurchLine."Line No.");
                        end else begin
                            ...
                            if GLSetup.CheckFullGSTonPrepayment("VAT Bus. Posting Group", "VAT Prod. Posting Group") then begin
                                ...
                                PurLine.Reset();
                                InvDiscAmount := 0;
                                PurLine.SetFilter("Document No.", "Document No.");
    ...
    begin
        with PurchLine do begin
            if PurchHeader."Prices Including VAT" then
                if GLSetup.CheckFullGSTonPrepayment("VAT Bus. Posting Group", "VAT Prod. Posting Group") then begin
                    GSTBase := Round("Prepmt. VAT Base Amt." * "Qty. to Invoice" / Quantity, Currency."Amount Rounding Precision");
                    GSTAmount := Round(GSTBase * "Prepayment VAT %" / 100, Currency."Amount Rounding Precision");
                    PrepmtVATBaseToDeduct := "Prepmt Amt to Deduct" - GSTAmount;
                end else
    ...
    begin
        ...
        with PrepmtPurchLine do begin
            ...
            if (PricesInclVATRoundingAmount[1] = 0) and (PricesInclVATRoundingAmount[2] = 0) or
               ("Currency Code" <> '') and FinalInvoice or
               GLSetup.CheckFullGSTonPrepayment("VAT Bus. Posting Group", "VAT Prod. Posting Group")
            then
                Increment(
                  TotalPurchLineLCY."Amount Including VAT",
    ...
    begin
        with GenJnlLine do begin
            ...
            "Currency Code" := PurchHeader."Currency Code";
            Amount := -WHTEntry.Amount;
            "Vendor Exchange Rate (ACY)" := PurchHeader."Vendor Exchange Rate (ACY)";
            "Amount Including VAT (ACY)" := -TotalPurchLineLCY."Amount Including VAT (ACY)";
            "Source Currency Code" := PurchHeader."Currency Code";
            "Source Currency Amount" := -WHTEntry.Amount;
            if PurchHeader."Currency Code" <> '' then
                "Amount (LCY)" :=
                  Round(
                    CurrExchRate.ExchangeAmtFCYToLCY(
    ...
    begin
        ...
        if TempInvoicePostBufferGST.FindSet() then
            repeat
                with TempInvoicePostBufferGST do begin
                    GSTPurchEntry.Init();
                        ...
                        PurchHeader."Document Type"::Invoice:
                            begin
                                ...
                                if PurchLine3.Get(PurchHeader."Document Type", PurchHeader."No.", "Fixed Asset Line No.") then begin
                                    ...
                                    if PurchInvLine3.Get(PurchInvHeader."No.", "Fixed Asset Line No.") then begin
                        ...
                        PurchHeader."Document Type"::"Credit Memo":
                            begin
                                ...
                                if PurchLine3.Get(PurchHeader."Document Type", PurchHeader."No.", "Fixed Asset Line No.") then begin
                                    ...
                                    if PurchCrMemoLine3.Get(PurchCrMemoHeader."No.", "Fixed Asset Line No.") then begin
    ...
    begin
        ...
        with PurchLine do begin
            ...
            if "Qty. to Invoice" <> 0 then begin
                InvoicePostBuffer.Type := Type;
                InvoicePostBuffer."Fixed Asset Line No." := "Line No.";
                if (Type = Type::"G/L Account") or (Type = Type::"Fixed Asset") then begin
                    InvoicePostBuffer."Entry Description" := Description;
                    InvoicePostBuffer."G/L Account" := "No.";
                end else begin
                ...
                InvoicePostBuffer."Gen. Bus. Posting Group" := "Gen. Bus. Posting Group";
                InvoicePostBuffer."Gen. Prod. Posting Group" := "Gen. Prod. Posting Group";
                InvoicePostBuffer."VAT Bus. Posting Group" := "VAT Bus. Posting Group";
                InvoicePostBuffer."VAT Prod. Posting Group" := "VAT Prod. Posting Group";
                InvoicePostBuffer."VAT Calculation Type" := "VAT Calculation Type";
                InvoicePostBuffer."Global Dimension 1 Code" := "Shortcut Dimension 1 Code";
                InvoicePostBuffer."Global Dimension 2 Code" := "Shortcut Dimension 2 Code";
                InvoicePostBuffer."Job No." := "Job No.";
                InvoicePostBuffer.Amount := Amount;
                InvoicePostBuffer."VAT Base Amount" := Amount;
                if "Prepayment Line" then begin
                    InvoicePostBuffer.Amount := Round("Line Amount", Currency."Amount Rounding Precision");
                    InvoicePostBuffer."VAT Base Amount" := Round("VAT Base Amount", Currency."Amount Rounding Precision");
                end;
                InvoicePostBuffer."Amount (ACY)" := PurchLineACY.Amount;
                InvoicePostBuffer."VAT Base Amount (ACY)" := PurchLineACY.Amount;
                InvoicePostBuffer."VAT Difference" := "VAT Difference";
                InvoicePostBuffer."VAT %" := "VAT %";
                InvoicePostBuffer.Adjustment := PurchHeader.Adjustment;
                InvoicePostBuffer."Deferral Code" := "Deferral Code";
                ...
                InvoicePostBuffer."VAT Base (ACY)" := "VAT Base (ACY)";
                InvoicePostBuffer."VAT Difference (ACY)" := "VAT Difference (ACY)";
                InvoicePostBuffer."Amount Including VAT (ACY)" := "Amount Including VAT (ACY)";

                if Type = Type::"Fixed Asset" then begin
                    InvoicePostBuffer."FA Posting Date" := "FA Posting Date";
                    InvoicePostBuffer."FA Posting Type" := "FA Posting Type";
                    InvoicePostBuffer."Depreciation Book Code" := "Depreciation Book Code";
                    InvoicePostBuffer."Salvage Value" := "Salvage Value";
                    InvoicePostBuffer."Depr. until FA Posting Date" := "Depr. until FA Posting Date";
                    InvoicePostBuffer."Depr. Acquisition Cost" := "Depr. Acquisition Cost";
                    InvoicePostBuffer."Maintenance Code" := "Maintenance Code";
                    InvoicePostBuffer."Insurance No." := "Insurance No.";
                    InvoicePostBuffer."Budgeted FA No." := "Budgeted FA No.";
                    InvoicePostBuffer."Duplicate in Depreciation Book" := "Duplicate in Depreciation Book";
                    InvoicePostBuffer."Use Duplication List" := "Use Duplication List";
                end;
                case "VAT Calculation Type" of
                    "VAT Calculation Type"::"Normal VAT", "VAT Calculation Type"::"Full VAT":
                        ...
                        then begin
                            if "Amount Including VAT" < Amount then begin
                                InvoicePostBuffer."VAT Amount" := Round("Amount Including VAT" -
                                ...
                                InvoicePostBuffer."VAT Amount (ACY)" := -Round(InvoicePostBuffer."VAT Base Amount" * "VAT %" / 100
                                    , Currency."Amount Rounding Precision");
                                InvoicePostBuffer."VAT Amount(ACY)" := -Round(InvoicePostBuffer."VAT Base Amount" * "VAT %" / 100
                                    , Currency."Amount Rounding Precision");
                            end;
                            ...
                            InvoicePostBuffer."VAT Amount" := "Amount Including VAT" - Amount;
                            InvoicePostBuffer."VAT Amount (ACY)" :=
                              PurchLineACY."Amount Including VAT (ACY)" - PurchLineACY.Amount;
                            InvoicePostBuffer."VAT Amount(ACY)" := "Amount Including VAT (ACY)" - "VAT Base (ACY)";
                        end;
                    ...
                    "VAT Calculation Type"::"Sales Tax":
                        begin
                            ...
                            InvoicePostBuffer."Tax Area Code" := "Tax Area Code";
                            InvoicePostBuffer."Tax Liable" := "Tax Liable";
                            InvoicePostBuffer."Tax Group Code" := "Tax Group Code";
                            InvoicePostBuffer."Use Tax" := "Use Tax";
                            InvoicePostBuffer.Quantity := "Qty. to Invoice (Base)";
                        end;
                end;
                case PurchSetup."Discount Posting" of
                    PurchSetup."Discount Posting"::"Invoice Discounts":
                        begin
                            InvoicePostBuffer.Amount := InvoicePostBuffer.Amount + "Inv. Discount Amount";
                    ...
                    PurchSetup."Discount Posting"::"Line Discounts":
                        begin
                            InvoicePostBuffer.Amount := InvoicePostBuffer.Amount + "Line Discount Amount";
                            InvoicePostBuffer."Amount (ACY)" :=
                    ...
                    PurchSetup."Discount Posting"::"All Discounts":
                        begin
                            InvoicePostBuffer.Amount :=
                              InvoicePostBuffer.Amount + "Line Discount Amount" + "Inv. Discount Amount";
                            InvoicePostBuffer."Amount (ACY)" :=
                              InvoicePostBuffer."Amount (ACY)" +
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            "VAT Base (ACY)" := InvoicePostBuffer."VAT Base (ACY)";
            "VAT Amount (ACY)" := InvoicePostBuffer."VAT Amount(ACY)";
            "VAT Difference (ACY)" := InvoicePostBuffer."VAT Difference (ACY)";
            "Amount Including VAT (ACY)" := InvoicePostBuffer."Amount Including VAT (ACY)";
            "Orig. Pmt. Disc. Possible" := TotalPurchLine."Pmt. Discount Amount";
            "Orig. Pmt. Disc. Possible(LCY)" :=
              CurrExchRate.ExchangeAmtFCYToLCY(
                PurchHeader.GetUseDate(), PurchHeader."Currency Code", TotalPurchLine."Pmt. Discount Amount", PurchHeader."Currency Factor");

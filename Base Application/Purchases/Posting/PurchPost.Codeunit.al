...
codeunit 90 "Purch.-Post"
{
    ...
    begin
        with PurchHeader do begin
            ...
            UpdPostingDescriptiononInvoice("Posting No.", PurchHeader, ModifyHeader);
...
#pragma warning restore AA0005
    ...
    begin
        ...
        with PurchLine do begin
            ...
            IsHandled := false;
            OnRoundAmountOnBeforeCalculateLCYAmounts(xPurchLine, PurchLineACY, PurchHeader, IsHandled);
            if not IsHandled then
                if PurchHeader."Currency Code" <> '' then begin
    ...
    begin
        ...
        with PurchaseHeader do
            if ("Posting Description" = Format("Document Type") + ' ' + "No.") or (PostingNo <> '')
            then begin
                if not ("Document Type" in ["Document Type"::"Credit Memo", "Document Type"::"Return Order"]) then
                    "Posting Description" :=
                      CopyStr(Text92000 + ' ' + "Vendor Invoice No." + '/' + "Pay-to Name", 1,
                        MaxStrLen("Posting Description"))
                else
                    "Posting Description" :=
                      CopyStr(Text92001 + ' ' + "Vendor Cr. Memo No." + '/' + "Pay-to Name", 1,
                        MaxStrLen("Posting Description"));
                ModifyHeader := true;
            end;
    end;

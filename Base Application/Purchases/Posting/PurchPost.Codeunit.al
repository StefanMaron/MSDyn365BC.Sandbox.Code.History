...
codeunit 90 "Purch.-Post"
{
    ...
    begin
        ...
        with PurchLine do begin
            ...
            if "Prepayment Line" and PurchHeader."Prepmt. Include Tax" then begin
            ...
            if IsCreditDocType() then begin
                ...
                if TaxOption = TaxOption::SalesTax then
                    if TempPurchLineForSalesTax.Get("Document Type", "Document No.", "Line No.") then begin
                ...
                if "Tax Area Code" <> '' then begin
                    ...
                    if Invoice then
                        TaxAmountDifference.ClearDocDifference(
                            Enum::"Sales Tax Document Area"::Purchase.AsInteger(), "Document Type".AsInteger(), "No.");
                    ...
                    if Invoice and "Prepmt. Include Tax" and ("Prepayment %" <> 0) and ("Currency Code" = '') and
    ...
    begin
        ...
        with PurchLine do
            if (PurchLineQty = 0) or ("Direct Unit Cost" = 0) and not
...
#pragma warning restore AA0005
        ...
        with PrepmtPurchLine do begin
            ...
            if FindSet() then
                repeat
                    ...
                    UpdatePrepmtAmountInvBuf("Line No.", ActualCurrencyFactor, PurchHeader."Prepmt. Include Tax");
                until Next() = 0;
            Reset();
        end;
    end;
    ...
    begin
        ...
        with TempICGenJnlLine do begin
            ...
            "Bal. Tax Area Code" := PurchLine."Tax Area Code";
            "Bal. Tax Liable" := PurchLine."Tax Liable";
            "Bal. Tax Group Code" := PurchLine."Tax Group Code";
            Validate("Bal. VAT %");
            if "Bal. VAT %" <> 0 then
    ...
    begin
        ...
        with TempPurchLineForSalesTax do begin
            ...
            if ("Prepayment %" = 100) and not "Prepayment Line" and ("Prepmt Amt to Deduct" <> 0) then
                "Line Amount" := "Prepmt Amt to Deduct" + "Inv. Disc. Amount to Invoice"
            else
                "Line Amount" := "Line Amount" - "Line Discount Amount";
            "Inv. Discount Amount" := "Inv. Disc. Amount to Invoice";
            Amount := CalcLineAmount();
    ...
    begin
        with GenJournalLine do begin
            ...
            "Account No." := VendorPostingGroup."Invoice Rounding Account";
            "Currency Code" := PurchaseHeader."Currency Code";
            Amount := LineAmount;
            "Source Currency Code" := PurchaseHeader."Currency Code";
            "Source Currency Amount" := -TotalPurchLine."Amount Including VAT";
            "Amount (LCY)" := LineAmountLCY;
            if PurchaseHeader."Currency Code" = '' then
                "Currency Factor" := 1
            else
                        ...
                        if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then begin
                            ...
                            if SalesTaxCountry <> SalesTaxCountry::NoTax then
                                TaxAmountDifference.CopyTaxDifferenceRecords(
                                  TaxAmountDifference."Document Product Area"::Purchase, "Document Type".AsInteger(), "No.",

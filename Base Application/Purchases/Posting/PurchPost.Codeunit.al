...
#pragma warning restore AA0470
        ...
        Text1130008: Label 'must not be prior to %1';
        Text12100: Label 'It is not possible to post document with a vendor with VAT exemption if an active VAT exemption doesn''t exist.';
        Text12101: Label 'The error is in line no. %1.';
#pragma warning disable AA0470
...
#pragma warning restore AA0470
    ...
    procedure CalcInvoice(var PurchHeader: Record "Purchase Header"; var ReverseChargeVAT: Boolean) NewInvoice: Boolean
    var
    ...
    begin
        ...
        TempPurchLine.SetRange("VAT Calculation Type", TempPurchLine."VAT Calculation Type"::"Reverse Charge VAT");
        ReverseChargeVAT := not TempPurchLine.IsEmpty();
        TempPurchLine.SetRange("VAT Calculation Type");
    ...
    begin
        ...
        IsHandled := false;
        OnCheckAndUpdateOnBeforePaymentsInsert(PurchHeader, IsHandled);
        if not IsHandled then
            if PurchHeader.Invoice then begin
                ...
                PaymentTermsLine.SetRange(Code, PurchHeader."No.");
                if PaymentTermsLine.Find('-') then
                    repeat
                        ...
                        if PurchHeader."Document Type" in [PurchHeader."Document Type"::Order, PurchHeader."Document Type"::Invoice] then begin
                            ...
                            PostedPayments.Code := PurchInvHeader."No.";
                        end else begin
                            PostedPayments.Type := PostedPayments.Type::"Credit Memo";
                            PostedPayments.Code := PurchCrMemoHeader."No.";
                        end;
                        ...
                        OnCheckAndUpdateOnBeforeInsertPostedPayments(PurchHeader, PaymentTermsLine, PostedPayments);
                        ...
                        OnCheckAndUpdateOnBeforeInsertPaymentJournalLine(PurchHeader, PaymentTermsLine, PaymentJournalLine);
                        PaymentJournalLine.Insert();
                    until PaymentTermsLine.Next() = 0
                else
                    if PurchHeader."Payment Terms Code" <> '' then
                        Error(Text1130003, PurchHeader.FieldCaption("Payment Terms Code"), PurchHeader."Payment Terms Code");
            end;
    ...
    /// </summary>
    /// <param name="PurchHeader">The purchase header of the document that is being posted.</param>
    procedure CheckPurchDocument(var PurchHeader: Record "Purchase Header"; var ReverseChargeVAT: Boolean)
    var
    ...
    begin
        ...
        PurchHeader.TestField("Operation Occurred Date", ErrorInfo.Create());
        PurchHeader.TestField("Operation Type", ErrorInfo.Create());
        if not PurchHeader.FindVATExemption(VATExemption, Check, true) then
            if Check then
                Error(Text12100);

        if PurchHeader."Applies-to Doc. No." <> '' then
            if PurchHeader."Applies-to Occurrence No." = 0 then
                Error(ErrorInfo.Create(StrSubstNo(Text1130007, PurchHeader.FieldCaption("Applies-to Occurrence No."))));
        ...
        if PurchHeader.Invoice then
            PurchHeader.Invoice := CalcInvoice(PurchHeader, ReverseChargeVAT);

        if PurchHeader.Invoice then
            ValidateIncludeInVATReport(PurchHeader);
    ...
    begin
        ...
        if PurchLine."Document Type" in [PurchLine."Document Type"::Order, PurchLine."Document Type"::Invoice, PurchLine."Document Type"::"Credit Memo"] then
            if VATPostingSetup.IsEUService(PurchLine."VAT Bus. Posting Group", PurchLine."VAT Prod. Posting Group") then begin
                if VATPostingSetup."Service Tariff No. Mandatory" then begin
                    ...
                    if PurchLine.IsInventoriableItem() then
                        PurchHeader.TestField("Transport Method");
                    PurchHeader.TestField("Payment Method Code");
                end;
            end else
                PurchLine.TestField("Service Tariff No.", '');
        ...
        if PurchLine.Quantity <> 0 then begin
            ...
            if not IsHandled then
                if not ApplicationAreaMgmt.IsSalesTaxEnabled() then begin
                    ...
                    PurchLine.TestField("VAT Identifier");
                end;
            IsHandled := false;
    ...
    begin
        ...
        if not PreviewMode and GuiAllowed() then
            if PurchHeader."Currency Code" = '' then begin
                if Abs(TotalPurchLineLCY."Amount Including VAT") <> PurchHeader."Check Total" then
                    Error(Text1130004, Abs(TotalPurchLineLCY."Amount Including VAT"), PurchHeader."Check Total");
            end else
                if Abs(TotalPurchLine."Amount Including VAT") <> PurchHeader."Check Total" then
                    Error(Text1130005, Abs(TotalPurchLine."Amount Including VAT"), PurchHeader."Check Total", PurchHeader."Currency Code");
...
#pragma warning restore AL0432
    ...
    begin
        ...
        if not IsHandled then
            if PurchHeader.Invoice and (PurchHeader."Posting No." = '') then begin
                ...
                then begin
                    ...
                    NoSeries.Get(PurchHeader."Posting No. Series");
                    NoSeries.TestField("Date Order");
                end;
                if (PurchHeader."No. Series" <> PurchHeader."Posting No. Series") or
                   (PurchHeader."Document Type" in [PurchHeader."Document Type"::Order, PurchHeader."Document Type"::"Return Order"])
                then begin
                    if PurchSetup."Ext. Doc. No. Mandatory" or (GenJnlLineExtDocNo <> '') then begin
                        VendLedgEntry.Reset();
                        VendLedgEntry.SetCurrentKey("External Document No.");
                        if (PurchHeader."Document Type" = PurchHeader."Document Type"::Invoice) or (PurchHeader."Document Type" = PurchHeader."Document Type"::Order) then begin
                            PurchHeader.TestField("Vendor Invoice No.");
                            VendorMgt.SetFilterForExternalDocNo(
                                VendLedgEntry, PurchHeader."Document Type"::Invoice, PurchHeader."Vendor Invoice No.", PurchHeader."Pay-to Vendor No.", PurchHeader."Document Date");
                        end else
                            if (PurchHeader."Document Type" = PurchHeader."Document Type"::"Credit Memo") or (PurchHeader."Document Type" = PurchHeader."Document Type"::"Return Order") then begin
                                PurchHeader.TestField("Vendor Cr. Memo No.");
                                VendorMgt.SetFilterForExternalDocNo(
                                    VendLedgEntry, PurchHeader."Document Type"::"Credit Memo", PurchHeader."Vendor Cr. Memo No.", PurchHeader."Pay-to Vendor No.", PurchHeader."Document Date");
                            end;
                        IsHandled := false;
                        OnUpdatePostingNosOnBeforeCheckExternalDocNo(VendLedgEntry, PurchHeader."Posting Date", IsHandled);
                        if not IsHandled then
        ...
        if PurchHeader.Invoice and ReverseChargeVAT and (PurchHeader."Reverse Sales VAT No." = '') then
            if PurchHeader."Reverse Sales VAT No. Series" <> '' then
                if not PreviewMode then begin
                    NoSeries.Get(PurchHeader."Reverse Sales VAT No. Series");
                    NoSeries.TestField("Date Order");
                    PurchHeader."Reverse Sales VAT No." := NoSeriesCodeunit.GetNextNo(PurchHeader."Reverse Sales VAT No. Series", PurchHeader."Posting Date");
                    ModifyHeader := true;
                end else
                    PurchHeader."Reverse Sales VAT No." := '***';
...
#pragma warning restore AA0005
    ...
    begin
        ...
        VendLedgEntry2.SetRange("Document No.", GenJnlLineDocNo);
        ...
        GenJnlLine."Operation Occurred Date" := GenJnlLine."Operation Occurred Date";
        ...
        GenJnlLine.Amount := TotalPurchLine2."Amount Including VAT" + TotalRemainPmtDiscPossible;
        ...
        if TempPurchLine.FindSet() then begin
            ...
            repeat
                ...
                if TempPurchLine."Prepmt Amt to Deduct" <> 0 then begin
                    ...
                    if not IsHandled then
                        GLAcc.Get(VATPostingSetup."Purch. Prepayments Account");
                    ...
                    if TempLineFound then begin
                        ...
                        TempPrepmtPurchLine."Include in VAT Transac. Rep." := not TempPrepmtPurchLine."Prepayment Line";
    ...
    [Scope('OnPrem')]
    procedure SplitFA(GenJnlLine: Record "Gen. Journal Line"; SplitNo: Integer)
    var
    ...
    begin
        ...
        for I := 1 to SplitNo do begin
            ...
            CalcSplitAmount(GenJnlLine.Amount, GenJnlLine2.Amount, TotalGenJnlLine.Amount, I, SplitNo);
            CalcSplitAmount(
              GenJnlLine."Source Currency Amount",
              GenJnlLine2."Source Currency Amount",
              TotalGenJnlLine."Source Currency Amount", I, SplitNo);
            CalcSplitAmount(GenJnlLine.Quantity, GenJnlLine2.Quantity, TotalGenJnlLine.Quantity, I, SplitNo);
            CalcSplitAmount(
              GenJnlLine."VAT Base Amount", GenJnlLine2."VAT Base Amount",
              TotalGenJnlLine."VAT Base Amount", I, SplitNo);
            CalcSplitAmount(
              GenJnlLine."Source Curr. VAT Amount",
              GenJnlLine2."Source Curr. VAT Amount",
              TotalGenJnlLine."Source Curr. VAT Amount", I, SplitNo);
            CalcSplitAmount(
              GenJnlLine."VAT Amount", GenJnlLine2."VAT Amount",
              TotalGenJnlLine."VAT Amount", I, SplitNo);
            CalcSplitAmount(
              GenJnlLine."Source Curr. VAT Amount",
              GenJnlLine2."Source Curr. VAT Amount",
              TotalGenJnlLine."Source Curr. VAT Amount", I, SplitNo);
            CalcSplitAmount(
              GenJnlLine."VAT Difference", GenJnlLine2."VAT Difference",
              TotalGenJnlLine."VAT Difference", I, SplitNo);
            CalcSplitAmount(
              GenJnlLine."Salvage Value", GenJnlLine2."Salvage Value",
              TotalGenJnlLine."Salvage Value", I, SplitNo);
            GenJnlPostLine.RunWithCheck(GenJnlLine);
        end;
    end;
    ...
    [Scope('OnPrem')]
    procedure CalcSplitAmount(var Amount: Decimal; var Amount2: Decimal; TotalAmount: Decimal; I: Integer; SplitNo: Integer)
    begin
        if I < SplitNo then
    ...
    local procedure ValidateIncludeInVATReport(PurchaseHeaderParam: Record "Purchase Header")
    var
    ...
    begin
        ...
        InclInVATReportValidation.ValidatePurchaseHeader(PurchaseHeaderParam, InclInVATReportErrorLogTemp);
        OnValidateIncludeInVATReportOnAfterValidatePurchaseHeader(PurchaseHeaderParam, InclInVATReportErrorLogTemp);
        if InclInVATReportErrorLogTemp.FindFirst() then begin
            if InclInVATReportErrorLogTemp."Line No." <> 0 then
                Error(InclInVATReportErrorLogTemp."Error Message" + ' ' + StrSubstNo(Text12101, InclInVATReportErrorLogTemp."Line No."));
...
#if not CLEAN23
    local procedure PostInvoicePostBufferLine(PurchHeader: Record "Purchase Header"; InvoicePostBuffer: Record "Invoice Post. Buffer"; var GenJnlLine: Record "Gen. Journal Line"): Integer
    begin
        ...
        GenJnlLine."Related Entry No." := InvoicePostBuffer."Related Entry No.";
        GenJnlLine."Orig. Pmt. Disc. Possible" := TotalPurchLine."Pmt. Discount Amount";
        GenJnlLine."Orig. Pmt. Disc. Possible(LCY)" :=
          CurrExchRate.ExchangeAmtFCYToLCY(
            PurchHeader.GetUseDate(), PurchHeader."Currency Code", TotalPurchLine."Pmt. Discount Amount", PurchHeader."Currency Factor");
        ...
        if InvoicePostBuffer.Type = InvoicePostBuffer.Type::"Fixed Asset" then begin
            ...
            Split := CalcSplitFA(GenJnlLine, InvoicePostBuffer."No. of Fixed Asset Cards");
        end;
    end;
#endif
    ...
    begin
        ...
        if TempInvoicePostBuffer.Find('+') then
            repeat
                ...
                PostInvoicePostBufferLine(PurchHeader, TempInvoicePostBuffer, GenJnlLine);

                if Split then
                    SplitFA(GenJnlLine, TempInvoicePostBuffer."No. of Fixed Asset Cards")
                else begin
                    OnBeforePostInvPostBuffer(GenJnlLine, TempInvoicePostBuffer, PurchHeader, GenJnlPostLine, PreviewMode, SuppressCommit, GenJnlLineDocNo);
                    GLEntryNo := RunGenJnlPostLine(GenJnlLine);
                    OnAfterPostInvPostBuffer(GenJnlLine, TempInvoicePostBuffer, PurchHeader, GLEntryNo, SuppressCommit, GenJnlPostLine);
                end;

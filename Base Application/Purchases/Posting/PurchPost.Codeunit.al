...
codeunit 90 "Purch.-Post"
{
    ...
    begin
        with PurchHeader do begin
            ...
            if Invoice and ("Operation Occurred Date" < GLSetup."Last Gen. Jour. Printing Date") then
                FieldError("Operation Occurred Date", StrSubstNo(Text1130008, GLSetup."Last Gen. Jour. Printing Date"));
            ...
            if not IsHandled then
                if Invoice then begin
                    ...
                    PaymentTermsLine.SetRange(Type, "Document Type");
                    PaymentTermsLine.SetRange(Code, "No.");
                    if PaymentTermsLine.Find('-') then
                        ...
                        if "Payment Terms Code" <> '' then
                            Error(Text1130003, FieldCaption("Payment Terms Code"), "Payment Terms Code");
                end;
        ...
        with PurchHeader do begin
            ...
            if "Applies-to Doc. No." <> '' then
                if "Applies-to Occurrence No." = 0 then
                    Error(ErrorInfo.Create(StrSubstNo(Text1130007, FieldCaption("Applies-to Occurrence No."))));
            ...
            if Invoice then
                Invoice := CalcInvoice(PurchHeader, ReverseChargeVAT);
    ...
    begin
        ...
        with PurchHeader do begin
                ...
                if Invoice and ("Posting No." = '') then begin
                    ...
                    then begin
                        if PurchSetup."Ext. Doc. No. Mandatory" or (GenJnlLineExtDocNo <> '') then begin
                            ...
                            IsHandled := false;
                            OnUpdatePostingNosOnBeforeCheckExternalDocNo(VendLedgEntry, "Posting Date", IsHandled);
                            if not IsHandled then
            ...
            if Invoice and ReverseChargeVAT and ("Reverse Sales VAT No." = '') then
                if "Reverse Sales VAT No. Series" <> '' then
                    if not PreviewMode then begin
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            if WithhSocSec.Get("Document Type", "No.") then
                PostWithhSocSec(PurchHeader);

            WithhSocSec.SetRange("Document Type", "Document Type");
            WithhSocSec.SetRange("No.", "No.");
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            if Get("Document Type", "No.") then begin
                "Reverse Sales VAT No." := '';
                Modify();
            end;
        end;
...
#pragma warning restore AA0005
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            "Operation Occurred Date" := "Operation Occurred Date";
            ...
            Amount := TotalPurchLine2."Amount Including VAT" + TotalRemainPmtDiscPossible;
    ...
    begin
        with PurchHeader do
            if Invoice then
                if WithhSocSec.Get("Document Type", "No.") and
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            for I := 1 to SplitNo do begin
                ...
                CalcSplitAmount(Amount, GenJnlLine2.Amount, TotalGenJnlLine.Amount, I, SplitNo);
                CalcSplitAmount(
                  "Source Currency Amount",
                  GenJnlLine2."Source Currency Amount",
                  TotalGenJnlLine."Source Currency Amount", I, SplitNo);
                CalcSplitAmount(Quantity, GenJnlLine2.Quantity, TotalGenJnlLine.Quantity, I, SplitNo);
                CalcSplitAmount(
                  "VAT Base Amount", GenJnlLine2."VAT Base Amount",
                ...
                CalcSplitAmount(
                  ...
                  TotalGenJnlLine."Source Curr. VAT Amount", I, SplitNo);
                CalcSplitAmount(
                  "VAT Difference", GenJnlLine2."VAT Difference",
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            "Deductible %" := InvoicePostBuffer."Deductible %";
            "VAT Identifier" := InvoicePostBuffer."VAT Identifier";
            "Include in VAT Transac. Rep." := InvoicePostBuffer."Include in VAT Transac. Rep.";
            "Refers to Period" := InvoicePostBuffer."Refers to Period";
            "Contract No." := InvoicePostBuffer."Contract No.";
            "Service Tariff No." := InvoicePostBuffer."Service Tariff No.";
            "Transport Method" := InvoicePostBuffer."Transport Method";
            "Related Entry No." := InvoicePostBuffer."Related Entry No.";
            "Orig. Pmt. Disc. Possible" := TotalPurchLine."Pmt. Discount Amount";
            "Orig. Pmt. Disc. Possible(LCY)" :=
              CurrExchRate.ExchangeAmtFCYToLCY(
                PurchHeader.GetUseDate(), PurchHeader."Currency Code", TotalPurchLine."Pmt. Discount Amount", PurchHeader."Currency Factor");

...
#pragma warning restore AA0074
    ...
    begin
        ...
        if TransportMethod.Get(PurchHeader."Transport Method") and TransportMethod."Port/Airport" then
            PurchHeader.TestField("Entry Point", ErrorInfo.Create());
        ...
        CheckCorrectedInvoiceNo(PurchHeader);

        if (PurchHeader.Receive or PurchHeader.Invoice) and (not PurchHeader.IsCreditDocType()) then begin
            PurchHeader.TestField("Payment Method Code", ErrorInfo.Create());
            PurchHeader.TestField("Payment Terms Code", ErrorInfo.Create());
        end;
        ...
        if PurchHeader.Invoice and not PurchHeader.IsCreditDocType() then begin
            PurchHeader.TestField("Due Date", ErrorInfo.Create());
            PaymentTerms.VerifyMaxNoDaysTillDueDate(PurchHeader."Due Date", PurchHeader."Document Date", PurchHeader.FieldCaption("Due Date"));
        end;
    ...
    begin
        IsHandled := false;
        OnBeforeCheckCorrectedInvoiceNo(PurchaseHeader, IsHandled);
        if IsHandled then
            exit;

        if PurchaseHeader.IsCreditDocType() and PurchaseHeader.Invoice then begin
            if PurchSetup."Correct. Doc. No. Mandatory" then
                PurchaseHeader.TestField("Corrected Invoice No.")
            else begin
                if PurchaseHeader."Correction Type" = PurchaseHeader."Correction Type"::Removal then
                    PurchaseHeader.TestField("Corrected Invoice No.");
                if PurchaseHeader."Corrected Invoice No." = '' then
                    if not ConfirmManagement.GetResponseOrDefault(Text1100000, false) then
                        Error(Text1100011);
            end;
            if (PurchaseHeader."Corrected Invoice No." <> '') and (PurchaseHeader."Posting Description" = '') then
                PurchaseHeader."Posting Description" := Format(Text1100104) + ' ' + PurchaseHeader."No."
        end;
    end;
    ...
    begin
        ...
        if PurchaseHeader."Prices Including VAT" then
            ItemJnlLine."Discount Amount" :=
                (PurchaseLine."Line Discount Amount" + PurchaseLine."Inv. Discount Amount" + PurchaseLine."Pmt. Discount Amount") /
        ...
        ItemJnlLine."Pmt. Discount Amount" := PurchaseLine."Pmt. Discount Amount";
    end;
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if PurchHeader."Currency Code" <> '' then
                ItemJnlLine2."Discount Amount" := Round(
                    CurrExchRate.ExchangeAmtFCYToLCY(
                      ...
                      (ItemChargePurchLine."Inv. Discount Amount" + ItemChargePurchLine."Line Discount Amount" + ItemChargePurchLine."Pmt. Discount Amount") *
                ...
                ItemJnlLine2."Discount Amount" := Round(
                    (ItemChargePurchLine."Line Discount Amount" + ItemChargePurchLine."Inv. Discount Amount" + ItemChargePurchLine."Pmt. Discount Amount") *
                    ItemJnlLine2."Invoiced Qty. (Base)" /
                    ItemChargePurchLine."Quantity (Base)" * TempItemChargeAssgntPurch."Qty. to Handle" / QtyToInvoice,
            ...
            ItemJnlLine2."Pmt. Discount Amount" :=
                ...
                TempItemChargeAssgntPurch."Qty. to Assign" / QtyToInvoice;
            if TempItemChargeAssgntPurch."Document Type" in [TempItemChargeAssgntPurch."Document Type"::"Return Order", TempItemChargeAssgntPurch."Document Type"::"Credit Memo"] then begin
    ...
    begin
        ...
        if (PurchLineQty = 0) or (PurchLine."Direct Unit Cost" = 0) then begin
            ...
            if PurchLine."VAT Calculation Type" = PurchLine."VAT Calculation Type"::"Sales Tax" then begin
                ...
                PurchLine."EC %" := TempVATAmountLine."EC %";
            end;
            ...
            if PurchHeader."Prices Including VAT" then begin
                if (TempVATAmountLine.CalcLineAmount() = 0) or (PurchLine."Line Amount" = 0) then begin
                    ...
                    TempVATAmountLineRemainder."VAT Amount" +=
                      ...
                      (PurchLine.CalcLineAmount() - PurchLine."Pmt. Discount Amount") /
                ...
                if PurchLine."VAT Calculation Type" = PurchLine."VAT Calculation Type"::"Full VAT" then begin
                    ...
                    PurchLine.Amount := PurchLine.CalcLineAmount() - PurchLine."Pmt. Discount Amount";
...
#pragma warning restore AA0005
    ...
    begin
        ...
        if PurchLineQty <> PurchLine.Quantity then begin
            ...
            PurchLine."Pmt. Discount Amount" := Round(PurchLine."Pmt. Discount Amount" * PurchLineQty / PurchLine.Quantity, Currency."Amount Rounding Precision");
        end;
    end;
    ...
    begin
        ...
        GenJnlLine."Purch. Invoice Type" := PurchHeader."Invoice Type";
        GenJnlLine."Purch. Cr. Memo Type" := PurchHeader."Cr. Memo Type";
        GenJnlLine."Purch. Special Scheme Code" := PurchHeader."Special Scheme Code";
        GenJnlLine."Correction Type" := PurchHeader."Correction Type";
        GenJnlLine."Corrected Invoice No." := PurchHeader."Corrected Invoice No.";
        GenJnlLine."Succeeded Company Name" := PurchHeader."Succeeded Company Name";
        GenJnlLine."Succeeded VAT Registration No." := PurchHeader."Succeeded VAT Registration No.";
        GenJnlLine."ID Type" := PurchHeader."ID Type";
        GenJnlLine."Do Not Send To SII" := PurchHeader."Do Not Send To SII";
    ...
    begin
        ...
        InvoicePostingParameters."Auto Document No." := AutoDocNo;
    end;
    ...
    begin
        ...
        PurchLineToPost."Pmt. Discount Amount" := Round(
            PurchLine."Pmt. Discount Amount" / PurchLine.Quantity * QtyToAssign,
            GLSetup."Amount Rounding Precision");
        ...
        PurchLine."Pmt. Discount Amount" := PurchLine."Pmt. Discount Amount" - PurchLineToPost."Pmt. Discount Amount";
    ...
    begin
        ...
        PurchLine4.Reset();
        PurchLine4.SetRange("Document Type", PurchHeader."Document Type");
        PurchLine4.SetRange("Document No.", PurchHeader."No.");
        ...
        if PurchHeader."Generate Autoinvoices" or PurchLine4.FindFirst() then begin
            PurchHeader."Generate Autoinvoices" := true;
            AutoDocNo := '';
            GLSetup.TestField("Autoinvoice Nos.");
            AutoDocNo := NoSeries.GetNextNo(GLSetup."Autoinvoice Nos.", PurchHeader."Posting Date");
    ...
    begin
        ...
        if PurchHeader."Generate Autocredit Memo" or PurchLine3.FindFirst() then begin
            PurchHeader."Generate Autocredit Memo" := true;
            AutoDocNo := '';
            GLSetup.TestField("Autocredit Memo Nos.");
            AutoDocNo := NoSeries.GetNextNo(GLSetup."Autocredit Memo Nos.", PurchHeader."Posting Date");
    ...
    begin
        ...
        then begin
            GenJnlLine."Generate AutoInvoices" := true;
            GenJnlLine."AutoDoc. No." := AutoDocNo;
        end else
            GenJnlLine."Generate AutoInvoices" := false;
    ...
    local procedure CalcPaymentDiscountPosting(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; PurchLineACY: Record "Purchase Line"; var InvoicePostBuffer: Record "Invoice Post. Buffer")
    begin
        case PurchLine."VAT Calculation Type" of
            PurchLine."VAT Calculation Type"::"Normal VAT", PurchLine."VAT Calculation Type"::"Full VAT",
          PurchLine."VAT Calculation Type"::"No Taxable VAT":
                    ...
                    TempInvoicePostBuffer."VAT Calculation Type"::"Reverse Charge VAT":
                        begin
                            ...
                            if not IsHandled then begin
                                ...
                                VATAmount := VATBaseAmount * VATPostingSetup."VAT+EC %" / 100;
                                VATAmountACY := VATBaseAmountACY * VATPostingSetup."VAT+EC %" / 100;
                                ...
                                if TempInvoicePostBufferReverseCharge.Find() then begin
                                    ...
                                    if (VATBaseAmount <> 0) and (VATAmount <> 0) then
                                    ...
                                    if (VATBaseAmountACY <> 0) and (VATAmountACY <> 0) then
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckCorrectedInvoiceNo(var PurchaseHeader: Record "Purchase Header"; var IsHandled: Boolean)
    begin
    end;

...
#pragma warning restore AA0470
    ...
    begin
        ...
        UpdPostingDescriptiononInvoice(PurchHeader."Posting No.", PurchHeader, ModifyHeader);
...
#pragma warning restore AA0005
    ...
    begin
        ...
        if TempItemChargeAssgntPurch.FindSet() then
            repeat
                ...
                ChargedItem.Get(TempItemChargeAssgntPurch."Item No.");
            until TempItemChargeAssgntPurch.Next() = 0;
    ...
    local procedure UpdPostingDescriptiononInvoice(PostingNo: Code[20]; var PurchaseHeader: Record "Purchase Header"; var ModifyHeader: Boolean)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeUpdPostingDescriptiononInvoice(PurchaseHeader, PostingNo, ModifyHeader, IsHandled);
        if IsHandled then
            exit;

        if (PurchaseHeader."Posting Description" = Format(PurchaseHeader."Document Type") + ' ' + PurchaseHeader."No.") or (PostingNo <> '') then begin
            if not (PurchaseHeader."Document Type" in [PurchaseHeader."Document Type"::"Credit Memo", PurchaseHeader."Document Type"::"Return Order"]) then
                PurchaseHeader."Posting Description" :=
                  CopyStr(Text92000 + ' ' + PurchaseHeader."Vendor Invoice No." + '/' + PurchaseHeader."Pay-to Name", 1,
                    MaxStrLen(PurchaseHeader."Posting Description"))
            else
                PurchaseHeader."Posting Description" :=
                  CopyStr(Text92001 + ' ' + PurchaseHeader."Vendor Cr. Memo No." + '/' + PurchaseHeader."Pay-to Name", 1,
                    MaxStrLen(PurchaseHeader."Posting Description"));
            ModifyHeader := true;
        end;
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdPostingDescriptiononInvoice(var PurchaseHeader: Record "Purchase Header"; PostingNo: Code[20]; var ModifyHeader: Boolean; var IsHandled: Boolean)
    begin
    end;

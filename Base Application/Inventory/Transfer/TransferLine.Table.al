...
#pragma warning restore AS0072
            ...
            begin
                if CurrFieldNo <> 0 then
                    TestStatusOpen();
                if "WIP Quantity" <> 0 then
                    TestField("Item No.");
                if (("WIP Quantity" * "WIP Qty. Shipped") < 0) or
                   (Abs("WIP Quantity") < Abs("WIP Qty. Shipped"))
                then
                    FieldError("WIP Quantity", StrSubstNo(Text002, FieldCaption("WIP Qty. Shipped")));
...
#pragma warning restore AS0072
            ...
            begin
                if "WIP Qty. To Ship" > "WIP Outstanding Qty." then
                    if "WIP Outstanding Qty." > 0 then
                        Error(Text005, "WIP Outstanding Qty.")
                    else
                        Error(Text006);
            end;
#endif
...
#pragma warning restore AA0074
    ...
    begin
        ...
        "Return Order" := TransHeader."Return Order";
    ...
    begin
        ...
        "WIP Outstanding Qty. (Base)" := UOMMgt.CalcBaseQty("WIP Outstanding Qty.", "Qty. per Unit of Measure");
        "Completely Shipped" := ("WIP Quantity" <> 0) and ("WIP Outstanding Qty." = 0);
    end;

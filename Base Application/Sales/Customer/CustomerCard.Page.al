        ...
        area(content)
        {
            group(General)
            {
                ...
                field("Exposure (LCY)"; Rec."Exposure (LCY)")
                {
                    ...
                    begin
                        CustLedgEntry.Reset();
                        CustLedgEntry.SetCurrentKey("Customer No.", "Bank Receipt", "Document Type", "Due Date",
                          "Currency Code", "Bank Receipt Issued", Open);
                        ...
                        Rec.CopyFilter("Exposure Filter", CustLedgEntry."Due Date");
                        CustLedgEntry.SetRange(Open, true);
                        Rec.CopyFilter("Currency Filter", CustLedgEntry."Currency Code");
                        Rec.CopyFilter("Global Dimension 1 Filter", CustLedgEntry."Global Dimension 1 Code");
                        Rec.CopyFilter("Global Dimension 2 Filter", CustLedgEntry."Global Dimension 2 Code");
                ...
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the identification number of the customer''s tax representative.';
                }
            ...
            group(Individual)
            {
                ...
                field("PEC E-Mail Address"; Rec."PEC E-Mail Address")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the e-mail address to identify the customer instead of the PA Code in the electronic invoice.';
                }
            }
            ...
            group(Payments)
            {
                ...
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies a code that specifies how the program will compute interest on arrears for the customer.';
                }
    ...
    actions
    {
        area(navigation)
        {
            ...
            group(Documents)
            {
                ...
                action("Stop Payment Periods")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Stop Payment Periods';
                    Image = StopPayment;
                    RunObject = Page "Stop Payment Periods";
                    RunPageLink = "No." = field("No.");
                    ToolTip = 'View or edit the list of the time periods in which payments are not allowed.';
                }
                action("Fixed Due Dates")
                {
                    ApplicationArea = Basic, Suite;
                    ...
                    RunPageLink = Type = const(Customer),
                                  Code = field("No.");
                    ToolTip = 'View or edit the list of numbers of days that define the period of time in which payments are allowed.';
                }
                action("VAT E&xemption")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'VAT E&xemption';
                    Image = VATExemption;
                    RunObject = Page "VAT Exemptions";
                    RunPageLink = Type = const(Customer),
                                  "No." = field("No.");
                    RunPageView = sorting(Type, "No.", "VAT Exempt. Starting Date", "VAT Exempt. Ending Date", "VAT Exempt. No.");
                    ToolTip = 'View or edit the list of VAT exemption time periods.';
                }
            }
            group("&Balance")
            {
                ...
                action("&Aging")
                {
                    ...
                    Caption = '&Aging';
                    Image = Aging;
                    RunObject = Page "Customer Aging";
                    ToolTip = 'Opens the Customer Aging report so you can see a list of amounts that will be due for payment from individual customers in the future.';
                }
            }
        }
        area(creation)

...
page 143 "Posted Sales Invoices"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Printed Tax Document"; Rec."Printed Tax Document")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies that a tax invoice is printed.';
                }
                field("Posted Tax Document"; Rec."Posted Tax Document")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies that a tax invoice is posted, or the option document post is selected for tax document type.';
                }
                field("Tax Document Marked"; Rec."Tax Document Marked")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies that the tax document type is selected as document post, or when we select to post a tax document.';
                }
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            group("Tax P&osting")
            {
                Caption = 'Tax P&osting';
                Image = Post;
                action("P&ost &Tax Invoice")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'P&ost &Tax Invoice';
                    Image = PostTaxInvoice;
                    ToolTip = 'Start the process of posting the sales tax invoice.';

                    trigger OnAction()
                    begin
                        TaxInvoiceMgt.GroupSalesTaxInvPosted(Rec, false);
                    end;
                }
                action("Post and &Print Tax Invoice")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Post and &Print Tax Invoice';
                    Image = PostPrint;
                    ToolTip = 'Start the process of posting and printing the tax invoice.';

                    trigger OnAction()
                    begin
                        TaxInvoiceMgt.GroupSalesTaxInvPosted(Rec, true);
                    end;
                }
            }
            group(Correct)
            {
                ...
                action(CancelInvoice)
                {
                    ...
                    Image = Cancel;
                    Scope = Repeater;
                    ToolTip = 'Create and post a sales credit memo that reverses this posted sales invoice. This posted sales invoice will be canceled. \\You can set up a default cancellation reason code on the Sales & Receivables Setup page.';
                    Visible = not Rec.Cancelled;
        ...
        area(Promoted)
        {
            group(Category_Process)
            {
                ...
                actionref("P&ost &Tax Invoice_Promoted"; "P&ost &Tax Invoice")
                {
                }
                actionref("Post and &Print Tax Invoice_Promoted"; "Post and &Print Tax Invoice")
                {
                }
    ...
    var
        TaxInvoiceMgt: Codeunit TaxInvoiceManagement;

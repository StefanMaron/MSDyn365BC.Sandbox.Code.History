    ...
    fields
    {
        ...
        field(4; "Bill-to Customer No."; Code[20])
        {
            ...
            begin
                ...
                "Cumulative Bank Receipts" := Customer."Cumulative Bank Receipts";
        ...
        field(20; "Posting Date"; Date)
        {
            ...
            begin
                ...
                if "Document Type" <> "Document Type"::Quote then begin
                    ...
                    if ("Document Date" > "Posting Date") and SalesReceivablesSetup."Link Doc. Date To Posting Date" then begin
                        IsHandled := false;
        ...
        field(27; "Shipment Method Code"; Code[10])
        {
            ...
            begin
                ...
                if "Shipment Method Code" <> '' then begin
                    ...
                    Validate("Exit Point", ShipmentMethod."Intra Shipping Code");
                end;
            end;
        }
...
#pragma warning restore AS0086
        ...
        field(12100; "Operation Type"; Code[20])
        {
            ...
            trigger OnLookup()
            begin
                OnBeforeLookupOperationType(Rec, xRec, GlobalNoSeries);
        ...
        field(12125; "Service Tariff No."; Code[10])
        {
            ...
            begin
                ...
                if ("Service Tariff No." <> xRec."Service Tariff No.") and
                   (xRec."Sell-to Customer No." = "Sell-to Customer No.")
                then
                    MessageIfSalesLinesExist(FieldCaption("Service Tariff No."));
            end;
        }
        ...
        field(12138; "Refers to Period"; Option)
        {
            ...
            begin
                if xRec."Refers to Period" <> "Refers to Period" then
                    MessageIfSalesLinesExist(FieldCaption("Refers to Period"));
            end;
        }
        field(12139; "Prepmt. CM Refers to Period"; Option)
        {
            ...
            begin
                if xRec."Prepmt. CM Refers to Period" <> "Prepmt. CM Refers to Period" then
                    MessageIfSalesLinesExist(FieldCaption("Prepmt. CM Refers to Period"));
            end;
        }
...
#pragma warning restore AA0470
        ...
        DeletingSplitVATLinesMsg: Label 'If you change %1, the existing sales lines will be deleted and new sales lines based on the new information on the header will be created.\\Automatically generated split VAT lines will be removed. If necessary, you can recreate them by choosing Generate Split VAT Lines.\\Do you want to change %1?', Comment = '%1=a field name or a table name whose value is just being changed';
    ...
    begin
        ...
        SplitVATLinesExist := GetSplitVATLines(SplitVATSalesLine);
        ...
        if Confirmed then begin
            RemoveSplitVATLinesIfExist(SplitVATSalesLine, SplitVATLinesExist);
            SalesLine.LockTable();
    ...
    begin
        ...
        if ShipmentMethod.ThirdPartyLoader("Shipment Method Code") then begin
            TestField("3rd Party Loader Type", ErrorInfo.Create());
            TestField("3rd Party Loader No.", ErrorInfo.Create());
        end else begin
            TestField("3rd Party Loader Type", "3rd Party Loader Type"::" ", ErrorInfo.Create());
            TestField("3rd Party Loader No.", '', ErrorInfo.Create());
        end;
        if ShippingAgent.ShippingAgentVendorOrContact("Shipping Agent Code") then
            TestField("TDD Prepared By", ErrorInfo.Create());
    end;
    ...
    begin
        ...
        exit(VATExemption.FindCustVATExemptionOnDate("Bill-to Customer No.", "Document Date", "Document Date"));
    end;
    ...
    begin
        "VAT Registration No." := GetVATRegistrationNo(CustNo);
    end;
    ...
    begin
        ...
        if GetSplitVATLines(SplitSalesLine) then begin
            if DIALOG.Confirm(RegenerateSplitVATLinesQst, true) then begin
                RemoveSplitVATLines(SplitSalesLine);
                AddSplitVATLines();
            end;
            exit;
        end;
    ...
    begin
        AddSplitVATLinesIgnoringALine(DummySalesLine);
    end;
    ...
    begin
        ...
        OnAddSplitVATLinesIgnoringALineOnAfterSetSplitSalesLineFilters(SplitSalesLine, Rec);
        if not SplitSalesLine.FindSet() then
            exit;

        CurrentLineNo := IncrementLineNo(GetHighestLineNo(SplitSalesLine));
        InitializeTotalingSalesLine(SplitSalesLine, TotalingSalesLine, CurrentLineNo);
    ...
    procedure GetSplitVATLines(var SplitSalesLine: Record "Sales Line"): Boolean
    begin
        SplitSalesLine.SetRange("Document Type", "Document Type");
        SplitSalesLine.SetRange("Document No.", "No.");
        SplitSalesLine.SetRange("Automatically Generated", true);
        exit(SplitSalesLine.FindSet());
    end;

    procedure RemoveSplitVATLines(var SplitSalesLine: Record "Sales Line")
    begin
        SplitSalesLine.DeleteAll();
    end;

    local procedure RemoveSplitVATLinesIfExist(var SplitSalesLine: Record "Sales Line"; LinesExist: Boolean)
    begin
        if LinesExist then
            RemoveSplitVATLines(SplitSalesLine);
    end;
    ...
    [Scope('OnPrem')]
    procedure UpdateTotalingSalesLine(SplitSalesLine: Record "Sales Line"; var TotalingSalesLine: Record "Sales Line")
    var
    ...
    [Scope('OnPrem')]
    procedure InitializeTotalingSalesLine(SplitSalesLine: Record "Sales Line"; var TotalingSalesLine: Record "Sales Line"; LineNo: Integer)
    var
        VATPostingSetup: Record "VAT Posting Setup";
        GLAccount: Record "G/L Account";
    begin
        TotalingSalesLine.Init();
        TotalingSalesLine.Validate("Document Type", SplitSalesLine."Document Type");
        TotalingSalesLine.Validate("Document No.", SplitSalesLine."Document No.");
        TotalingSalesLine.Validate(Type, TotalingSalesLine.Type::"G/L Account");
        TotalingSalesLine.Validate("Line No.", LineNo);
        TotalingSalesLine.Validate("Sell-to Customer No.", SplitSalesLine."Sell-to Customer No.");

        // The field 'Quantity' is -1 because the auto-generated line is intended to reverse the VAT corresponsing to the group of
        // other lines
        TotalingSalesLine.Validate(Quantity, -1);

        // Get parameters from VAT Posting Setup
        GetVATPostingSetup(VATPostingSetup, SplitSalesLine);
        TotalingSalesLine."VAT Prod. Posting Group" := VATPostingSetup."VAT Prod. Posting Group";
        TotalingSalesLine."VAT Bus. Posting Group" := VATPostingSetup."VAT Bus. Posting Group";
        TotalingSalesLine."VAT Calculation Type" := VATPostingSetup."VAT Calculation Type";
        TotalingSalesLine."No." := VATPostingSetup."Sales VAT Account";
        if GLAccount.Get(VATPostingSetup."Sales VAT Account") then
            TotalingSalesLine.Description := GLAccount.Name;
        TotalingSalesLine."Gen. Bus. Posting Group" := SplitSalesLine."Gen. Bus. Posting Group";
        TotalingSalesLine."Gen. Prod. Posting Group" := SplitSalesLine."Gen. Prod. Posting Group";
        TotalingSalesLine."VAT Identifier" := SplitSalesLine."VAT Identifier";
        TotalingSalesLine."Automatically Generated" := true;
        TotalingSalesLine.CreateDimFromDefaultDim(0);

        OnAfterInitializeTotalingSalesLine(TotalingSalesLine, SplitSalesLine, VATPostingSetup);
    end;

    [Scope('OnPrem')]
    procedure GetVATPostingSetup(var VATPostingSetup: Record "VAT Posting Setup"; SplitSalesLine: Record "Sales Line")
    begin
        VATPostingSetup.Reset();
        VATPostingSetup.SetRange("Reversed VAT Bus. Post. Group", SplitSalesLine."VAT Bus. Posting Group");
        VATPostingSetup.SetRange("Reversed VAT Prod. Post. Group", SplitSalesLine."VAT Prod. Posting Group");

        if not VATPostingSetup.FindFirst() then
            Error(MissingVATPostingSetupErr,
              SplitSalesLine.FieldCaption("VAT Prod. Posting Group"),
              SplitSalesLine."VAT Prod. Posting Group",
              VATPostingSetup.TableCaption(),
              VATPostingSetup.FieldCaption("Reversed VAT Bus. Post. Group"),
              SplitSalesLine."VAT Bus. Posting Group",
              VATPostingSetup.FieldCaption("Reversed VAT Prod. Post. Group"));
    end;

    [Scope('OnPrem')]
    procedure GetHighestLineNo(SplitSalesLine: Record "Sales Line"): Integer
    var
        LocalSalesLine: Record "Sales Line";
    begin
        LocalSalesLine.SetCurrentKey("Document Type", "Document No.", "Line No.");
        LocalSalesLine.SetRange("Document Type", SplitSalesLine."Document Type");
        LocalSalesLine.SetRange("Document No.", SplitSalesLine."Document No.");
    ...
    begin
        if SplitVATLinesExist then
            ...
            if HasItemChargeAssignment() then
                UserMessage := StrSubstNo(ResetItemChargeAssignMsg, ChangedFieldName)
            else
                UserMessage := StrSubstNo(RecreateSalesLinesMsg, ChangedFieldName);
    end;
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            "Fiscal Code" := SellToCustomer."Fiscal Code";
            "Place of Birth" := SellToCustomer."Place of Birth";
            if not SkipSellToContact then
    ...
    begin
        ...
        "Individual Person" := BillToCustomer."Individual Person";
        Resident := BillToCustomer.Resident;
        "First Name" := BillToCustomer."First Name";
        "Last Name" := BillToCustomer."Last Name";
        "Fiscal Code" := BillToCustomer."Fiscal Code";
        "Date of Birth" := BillToCustomer."Date of Birth";
        "Tax Representative Type" := BillToCustomer."Tax Representative Type";
        "Tax Representative No." := BillToCustomer."Tax Representative No.";
        if not SkipBillToContact then
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeValidateOperationType(var SalesHeader: Record "Sales Header"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnAfterInitInsert()
    begin
    end;

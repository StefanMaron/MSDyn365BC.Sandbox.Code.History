    ...
    layout
    {
        area(content)
        {
                    ...
                    group(Control90)
                    {
                        ...
                        group(Control4)
                        {
                            ...
                            field("TDD Prepared By"; Rec."TDD Prepared By")
                            {
                                ...
                                Editable = ShipToOptions = ShipToOptions::"Custom Address";
                                Importance = Additional;
                                ToolTip = 'Specifies the user ID of the transport delivery document (TDD) for the sales order.';
                            }
                        }
            ...
            group(Control1900201301)
            {
                ...
                {
                    ApplicationArea = Prepayments;
                    ToolTip = 'Specifies the time that is used to process and filter the prepayment credit memo transactions.';

                    trigger OnValidate()
                    begin
                        PrepmtCMReferstoPeriodOnAfterV();
                    end;
                }
            }
            group(Individual)
                ...
                group("Prepa&yment")
                {
                    ...
                    action(PostPrepaymentCreditMemo)
                    {
                        ...
                        var
                            SalesPostYNPrepmt: Codeunit "Sales-Post Prepayment (Yes/No)";
                        begin
                            Rec.TestField("Prepmt. CM Refers to Period");
                            if ApprovalsMgmt.PrePostApprovalCheckSales(Rec) then
...
#pragma warning restore AA0074
    ...
    local procedure PrepmtCMReferstoPeriodOnAfterV()
    begin
        CurrPage.Update();
    end;

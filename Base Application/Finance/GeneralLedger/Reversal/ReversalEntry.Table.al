...
#pragma warning restore AA0074
    ...
    begin
        ...
        SourceCodeSetup.Get();
        if (GLRegister."Journal Batch Name" = '') or
           (GLRegister."Source Code" in [SourceCodeSetup."Sales Entry Application", SourceCodeSetup."Purchase Entry Application"])
        then
    ...
    begin
        IsHandled := false;
        OnBeforeCheckPostingDate(PostingDate, CopyStr(TableCaption, 1, 50), EntryNo, JnlTemplName, IsHandled, Rec, MaxPostingDate);
        if IsHandled then
            exit;
    ...
    begin
        ...
        repeat
            ...
            if GlobalGLEntry.FindSet() then
                repeat
                    if GlobalGLEntry."Remaining Amount" <> GlobalGLEntry.Amount then begin
                        if GlobalGLEntry.Open then
                            Error(Text000, GlobalGLEntry.TableCaption(), GlobalGLEntry."Entry No.");
                        Error(Text006, GlobalGLEntry.TableCaption(), GlobalGLEntry."Entry No.");
                    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckPostingDate(PostingDate: Date; Caption: Text[50]; EntryNo: Integer; JnlTemplName: Code[10]; var IsHandled: Boolean; var ReversalEntry: Record "Reversal Entry"; var MaxPostingDate: Date)
    begin
    end;

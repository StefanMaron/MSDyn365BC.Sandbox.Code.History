...
table 179 "Reversal Entry"
{
    ...
    var
        ...
        GlobalWHTEntry: Record "WHT Entry";
...
#pragma warning restore AA0074
    ...
    begin
        ...
        InsertFromWHTEntry(TempTransactionNoInteger, Number, RevType, NextLineNo);
    ...
    begin
        ...
        if GlobalWHTEntry.Find('-') then
            repeat
                CheckWHT(GlobalWHTEntry);
            until GlobalWHTEntry.Next() = 0;
    ...
    begin
        ...
        if RevType = RevType::Transaction then begin
            ...
            GlobalWHTEntry.SetCurrentKey("Transaction No.");
            ...
            GlobalWHTEntry.SetRange("Transaction No.", Number);
        end else begin
            ...
            GlobalWHTEntry.SetRange("Entry No.", GlobalGLRegister."From WHT Entry No.", GlobalGLRegister."To WHT Entry No.");
        end;
    ...
    begin
        ...
        case "Entry Type" of
            ...
            "Entry Type"::WHT:
                exit(StrSubstNo('%1', GlobalWHTEntry.TableCaption()));
            else begin
    ...
    protected procedure InsertFromWHTEntry(var TempTransactionNoInteger: Record "Integer" temporary; Number: Integer; RevType: Option Transaction,Register; var NextLineNo: Integer)
    begin
        TempTransactionNoInteger.FindSet();
        repeat
            if RevType = RevType::Transaction then
                GlobalWHTEntry.SetRange("Transaction No.", TempTransactionNoInteger.Number);
            if GlobalWHTEntry.Find('-') then
                repeat
                    Clear(TempReversalEntry);
                    if RevType = RevType::Register then
                    ...
                    TempReversalEntry."Entry No." := GlobalWHTEntry."Entry No.";
                    TempReversalEntry."Posting Date" := GlobalWHTEntry."Posting Date";
                    TempReversalEntry."Source Code" := GlobalWHTEntry."Source Code";
                    TempReversalEntry."Transaction No." := GlobalWHTEntry."Transaction No.";
                    TempReversalEntry.Amount := GlobalWHTEntry.Amount;
                    TempReversalEntry."Amount (LCY)" := GlobalWHTEntry.Amount;
                    TempReversalEntry."Document Type" := GlobalWHTEntry."Document Type";
                    TempReversalEntry."Document No." := GlobalWHTEntry."Document No.";
                    TempReversalEntry."Line No." := NextLineNo;
                    NextLineNo := NextLineNo + 1;
                    TempReversalEntry.Insert();
                until GlobalWHTEntry.Next() = 0;

...
#pragma warning disable AA0470
        ...
        Text013: Label 'Payment without invoice is not allowed for line %1.';
#pragma warning restore AA0470
#pragma warning restore AA0074
    ...
    begin
        ...
        if GLSetup."Force Payment With Invoice" = true then
            if ((GenJnlLine."Document Type" in [GenJnlLine."Document Type"::Payment, GenJnlLine."Document Type"::" "]) and
                (GenJnlLine."Account Type" = GenJnlLine."Account Type"::Vendor)) then
                if GenJnlLine."Applies-to ID" = '' then
                    if ((GenJnlLine."Applies-to Doc. Type" = GenJnlLine."Applies-to Doc. Type"::Invoice) and (GenJnlLine."Applies-to Doc. No." = '')) or
                       ((GenJnlLine."Applies-to Doc. Type" = GenJnlLine."Applies-to Doc. Type"::" ") and (GenJnlLine."Applies-to Doc. No." = ''))
                    then
                        Error(ErrorInfo.Create(StrSubstNo(Text013, GenJnlLine."Line No."), true, GenJnlLine));

        if GenJnlLine."BAS Doc. No." <> '' then begin
            BASCalcSheet.SetRange(A1, GenJnlLine."BAS Doc. No.");
            BASCalcSheet.Find('-');
        end;
                ...
                begin
                    if (GenJnlLine."Account Type" = GenJnlLine."Account Type"::Customer) or
                       ((GenJnlLine."Account Type" = GenJnlLine."Account Type"::Vendor) and
                        (GenJnlLine."Gen. Posting Type" <> GenJnlLine."Gen. Posting Type"::Settlement)) or
